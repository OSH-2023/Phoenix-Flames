...
pub fn receiveSignal(thread: *mut tcb_t, cap: cap_t, isBlocking: word_t) {
    ...
    if tmp == NtfnState_Waiting as u64{
        ...
        if isBlocking!=0 {
            unsafe{
                thread_state_ptr_set_tsType(&mut (*thread).tcbState, ThreadState_BlockedOnNotification as u64);
...
pub fn cancelIPC(tptr:*mut tcb_t) {
    ...
    if tmp == ThreadState_BlockedOnReceive as u64{
    ...
    if tmp == ThreadState_BlockedOnNotification as u64{
        cancelSignal(tptr, thread_state_ptr_get_blockingObject(state) as *mut notification_t);
    }else
    if tmp == ThreadState_BlockedOnReply as u64{
