#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/api/faults.c"
/*
...
compile_assert(InvalidRoot, lookup_fault_invalid_root + 1 == seL4_InvalidRoot)
    compile_assert(MissingCapability, lookup_fault_missing_capability + 1 ==
                                          seL4_MissingCapability)
        compile_assert(DepthMismatch,
                       lookup_fault_depth_mismatch + 1 == seL4_DepthMismatch)
            compile_assert(GuardMismatch, lookup_fault_guard_mismatch + 1 ==
                                              seL4_GuardMismatch)
                compile_assert(seL4_UnknownSyscall_Syscall,
                               (word_t)n_syscallMessage ==
                                   seL4_UnknownSyscall_Syscall)
                    compile_assert(seL4_UserException_Number,
                                   (word_t)n_exceptionMessage ==
                                       seL4_UserException_Number)
                        compile_assert(seL4_UserException_Code,
                                       (word_t)n_exceptionMessage + 1 ==
                                           seL4_UserException_Code)

                            static inline unsigned int setMRs_lookup_failure(
                                tcb_t *receiver, word_t *receiveIPCBuffer,
...
bool_t handleFaultReply(tcb_t *receiver, tcb_t *sender) {
  ...
  seL4_MessageInfo_t tag =
      messageInfoFromWord(getRegister(sender, msgInfoRegister));
  ...
  switch (seL4_Fault_get_seL4_FaultType(fault)) {
    ...
    case seL4_Fault_UnknownSyscall:
      copyMRsFaultReply(sender, receiver, MessageID_Syscall,
                        MIN(length, n_syscallMessage));
      return (label == 0);

    case seL4_Fault_UserException:
      copyMRsFaultReply(sender, receiver, MessageID_Exception,
                        MIN(length, n_exceptionMessage));
      return (label == 0);

#ifdef CONFIG_KERNEL_MCS
    case seL4_Fault_Timeout:
      copyMRsFaultReply(sender, receiver, MessageID_TimeoutReply,
    ...
    case seL4_Fault_DebugException: {
      ...
      if (seL4_Fault_DebugException_get_exceptionReason(fault) !=
          seL4_SingleStep) {
...
word_t setMRs_fault(tcb_t *sender, tcb_t *receiver, word_t *receiveIPCBuffer) {
  switch (seL4_Fault_get_seL4_FaultType(sender->tcbFault)) {
    ...
    case seL4_Fault_UnknownSyscall: {
      ...
      return setMR(
          receiver, receiveIPCBuffer, n_syscallMessage,
    ...
    case seL4_Fault_DebugException: {
      word_t reason =
          seL4_Fault_DebugException_get_exceptionReason(sender->tcbFault);

      setMR(receiver, receiveIPCBuffer, seL4_DebugException_FaultIP,
            getRestartPC(sender));
      ...
      if (reason != seL4_SingleStep && reason != seL4_SoftwareBreakRequest) {
        ret = setMR(
            receiver, receiveIPCBuffer, seL4_DebugException_TriggerAddress,
            seL4_Fault_DebugException_get_breakpointAddress(sender->tcbFault));

        /* Breakpoint messages also set a "breakpoint number" register. */
        ret = setMR(
            receiver, receiveIPCBuffer, seL4_DebugException_BreakpointNumber,
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/api/syscall.c"
/*
...
exception_t handleUnknownSyscall(word_t w) {
  ...
  if (w == SysDebugCapIdentify) {
    ...
    lookupCapAndSlot_ret_t lu_ret =
        lookupCapAndSlot(NODE_STATE(ksCurThread), cptr);
  ...
  if (w == SysDebugRun) {
    ((void (*)(void *))getRegister(NODE_STATE(ksCurThread), capRegister))(
        (void *)getRegister(NODE_STATE(ksCurThread), msgInfoRegister));
    return EXCEPTION_NONE;
  }
#endif
  ...
  if (w == SysX86DangerousWRMSR) {
    ...
    if (CONFIG_WORD_SIZE == 32) {
      val = (uint64_t)getSyscallArg(0, NULL) |
            ((uint64_t)getSyscallArg(1, NULL) << 32);
    } else {
...
static exception_t handleInvocation(bool_t isCall, bool_t isBlocking,
                                    bool_t canDonate, bool_t firstPhase,
                                    cptr_t cptr)
#else
static exception_t handleInvocation(bool_t isCall, bool_t isBlocking)
#endif
{
...
#ifdef CONFIG_KERNEL_MCS
  status = decodeInvocation(seL4_MessageInfo_get_label(info), length, cptr,
                            lu_ret.slot, lu_ret.cap, isBlocking, isCall,
                            canDonate, firstPhase, buffer);
#else
  status =
      decodeInvocation(seL4_MessageInfo_get_label(info), length, cptr,
                       lu_ret.slot, lu_ret.cap, isBlocking, isCall, buffer);
#endif
  ...
  if (unlikely(thread_state_get_tsType(thread->tcbState) ==
               ThreadState_Restart)) {
    if (isCall) {
...
static inline void mcsPreemptionPoint(void) {
  /* at this point we could be handling a timer interrupt which actually ends
   * the current threads timeslice. However, preemption is possible on revoke,
   * which could have deleted the current thread and/or the current scheduling
   * context, rendering them invalid. */
  if (isSchedulable(NODE_STATE(ksCurThread))) {
    /* if the thread is schedulable, the tcb and scheduling context are still
     * valid */
    checkBudget();
  } else if (NODE_STATE(ksCurSC)->scRefillMax) {
    /* otherwise, if the thread is not schedulable, the SC could be valid -
     * charge it if so */
...
#define handleInvocation(isCall, isBlocking, canDonate, firstPhase, cptr) \
...
static void handleYield(void) {
  ...
  /* Manually updated the scConsumed so that the full timeslice isn't added,
   * just what was consumed */
...
exception_t handleSyscall(syscall_t syscall) {
  ...
  MCS_DO_IF_BUDGET({
    switch (syscall) {
      case SysSend:
        ret =
            handleInvocation(false, true, false, false,
      ...
      case SysNBSend:
        ret =
            handleInvocation(false, false, false, false,
                             getRegister(NODE_STATE(ksCurThread), capRegister));
        if (unlikely(ret != EXCEPTION_NONE)) {
          mcsPreemptionPoint();
          irq = getActiveIRQ();
      ...
      case SysCall:
        ret =
            handleInvocation(true, true, true, false,
                             getRegister(NODE_STATE(ksCurThread), capRegister));
        if (unlikely(ret != EXCEPTION_NONE)) {
          mcsPreemptionPoint();
          irq = getActiveIRQ();
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/c_traps.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static void NORETURN vmlaunch_failed(word_t failInvalid, word_t failValid) {
  ...
  if (failValid) {
    userError("vmlaunch/vmresume error %d",
              (int)vmread(VMX_DATA_INSTRUCTION_ERROR));
  }
...
void VISIBLE NORETURN restore_user_context(void) {
  ...
  if (ARCH_NODE_STATE(x86KSPendingInterrupt) != int_invalid) {
    ...
    asm volatile(
        ...
        :
        : [stack_top] "r"(&(kernel_stack_alloc[CURRENT_CPU_INDEX()][BIT(
              CONFIG_KERNEL_STACK_BITS)])),
...
#ifdef CONFIG_VTX
  if (thread_state_ptr_get_tsType(&cur_thread->tcbState) ==
  ...
  if (likely(cur_thread->tcbArch.tcbContext.registers[Error] == -1) &&
    ...
    if (config_set(CONFIG_KERNEL_SKIM_WINDOW)) {
      /* if we are using the SKIM window then we are trying to hide kernel state
       * from the user in the case of Meltdown where the kernel region is
       * effectively readable by the user. To prevent a storage channel across
       * threads through the irq stack, which is idirectly controlled by the
       * user, we need to clear the stack. We perform this here since when we
       * return *from* an interrupt we must use this stack and cannot clear it.
       * This means if we restore from interrupt, then enter from a syscall and
       * switch to a different thread we must either on syscall entry, or before
       * leaving the kernel, clear the irq stack. */
      irqstack[0] = 0;
      irqstack[1] = 0;
      irqstack[2] = 0;
      irqstack[3] = 0;
...
#endif
      asm volatile(
          // Set our stack pointer to the top of the tcb so we can efficiently
          // pop
          "movq %0, %%rsp\n"
          "popq %%rdi\n"
          "popq %%rsi\n"
...
#if defined(ENABLE_SMP_SUPPORT) && defined(CONFIG_KERNEL_SKIM_WINDOW)
              ,
          "c"(user_cr3), [scratch_offset] "i"(nodeSkimScratchOffset)
#endif
        // Clobber memory so the compiler is forced to complete all stores
        // before running this assembler
        : "memory");
...
void VISIBLE NORETURN c_x64_handle_interrupt(int irq, int syscall) {
  ...
  /* In the case of an interrupt the NextIP and the FaultIP should be the same
   * value, i.e. the address of the instruction the CPU was about to execute
   * before the interrupt. This is the 5th value pushed on by the hardware, so
   * indexing from the bottom is x64KSIRQStack[1] */
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/kernel/elf.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE bool_t elf_checkFile(Elf64_Header_t *elf) {
  return (elf->e_ident[0] == '\177' && elf->e_ident[1] == 'E' &&
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/kernel/thread.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE void Arch_configureIdleThread(tcb_t *tcb) {
   ...
   * allows us to distinguish an interrupt from the idle thread from an
   * interrupt from kernel execution, just by examining the saved RSP value
   * (since the kernel thread will have a valid RSP, and never 0). See traps.S
...
void Arch_activateIdleThread(tcb_t *tcb) { /* Don't need to do anything */ }
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/kernel/vspace.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
/* When using the SKIM window to isolate the kernel from the user we also need
 * to not use global mappings as having global mappings and entries in the TLB
 * is equivalent, for the purpose of exploitation, to having the mappings in the
 * kernel window */
...
pml4e_t boot_pml4[BIT(PML4_INDEX_BITS)] ALIGN(
    BIT(seL4_PageBits)) VISIBLE PHYS_BSS;
pdpte_t boot_pdpt[BIT(PDPT_INDEX_BITS)] ALIGN(
    BIT(seL4_PageBits)) VISIBLE PHYS_BSS;
...
BOOT_CODE bool_t map_kernel_window(uint32_t num_ioapic, paddr_t *ioapic_paddrs,
                                   uint32_t num_drhu, paddr_t *drhu_list) {
  uint64_t paddr;
  uint64_t vaddr;
  ...
  assert(
      IS_ALIGNED(KERNEL_ELF_BASE - KERNEL_ELF_PADDR_BASE, seL4_HugePageBits));
  assert(IS_ALIGNED(KDEV_BASE, seL4_HugePageBits));
  /* place the PDPT into the PML4 */
  x64KSKernelPML4[GET_PML4_INDEX(PPTR_BASE)] =
      pml4e_new(0,                                  /* xd */
                kpptr_to_paddr(x64KSKernelPDPT), 0, /* accessed */
                0,                                  /* cache_disabled */
                0,                                  /* write_through */
                0,                                  /* super_user */
                1,                                  /* read_write */
  ...
  x64KSKernelPDPT[GET_PDPT_INDEX(KERNEL_ELF_BASE)] =
      pdpte_pdpte_1g_new(0,                  /* xd */
                         PADDR_BASE, 0,      /* PAT */
  ...
  paddr = 0;
  vaddr = PPTR_BASE;
  for (paddr = 0; paddr < PADDR_TOP; paddr += BIT(seL4_HugePageBits)) {
    int pdpte_index = GET_PDPT_INDEX(vaddr);
    x64KSKernelPDPT[pdpte_index] =
        pdpte_pdpte_1g_new(0,                  /* xd               */
  ...
  x64KSKernelPDPT[GET_PDPT_INDEX(KDEV_BASE)] =
      pdpte_pdpte_pd_new(0,                                /* xd */
                         kpptr_to_paddr(x64KSKernelPD), 0, /* accessed */
                         0,                                /* cache_disabled */
                         0,                                /* write_through */
                         0,                                /* super_user */
                         1,                                /* read_write */
  ...
  x64KSKernelPD[0] =
      pde_pde_pt_new(0,                                /* xd */
                     kpptr_to_paddr(x64KSKernelPT), 0, /* accessed */
                     0,                                /* cache_disabled */
                     0,                                /* write_through */
                     0,                                /* super_user */
                     1,                                /* read_write */
  ...
  for (pd_index = 0; pd_index < PADDR_TOP >> seL4_HugePageBits; pd_index++) {
    ...
    x64KSKernelPDPT[GET_PDPT_INDEX(PPTR_BASE) + pd_index] = pdpte_pdpte_pd_new(
        0,                                               /* xd */
        kpptr_to_paddr(&x64KSKernelPDs[pd_index][0]), 0, /* accessed */
        0,                                               /* cache disabled */
        0,                                               /* write through */
        0,                                               /* super user */
        1,                                               /* read write */
  ...
  x64KSKernelPDPT[GET_PDPT_INDEX(KERNEL_ELF_BASE)] = pdpte_pdpte_pd_new(
      0,                                        /* xd */
      kpptr_to_paddr(&x64KSKernelPDs[0][0]), 0, /* accessed */
      0,                                        /* cache disable */
      1,                                        /* write through */
      0,                                        /* super user */
      1,                                        /* read write */
  ...
  for (paddr = 0; paddr < PADDR_TOP; paddr += 0x200000) {
    int pd_index = GET_PDPT_INDEX(vaddr) - GET_PDPT_INDEX(PPTR_BASE);
    int pde_index = GET_PD_INDEX(vaddr);

    x64KSKernelPDs[pd_index][pde_index] =
        pde_pde_large_new(0,                  /* xd */
                          paddr, 0,           /* pat */
                          KERNEL_IS_GLOBAL(), /* global */
                          0,                  /* dirty */
                          0,                  /* accessed */
  ...
  /* put the PT into the PD */
  x64KSKernelPDs[BIT(PDPT_INDEX_BITS) - 1][0] =
      pde_pde_pt_new(0,                                /* xd */
                     kpptr_to_paddr(x64KSKernelPT), 0, /* accessed */
                     0,                                /* cache_disabled */
                     0,                                /* write_through */
  ...
  if (!map_kernel_window_devices(x64KSKernelPT, num_ioapic, ioapic_paddrs,
                                 num_drhu, drhu_list)) {
    return false;
  }
...
BOOT_CODE void init_syscall_msrs(void) {
  ...
  x86_wrmsr(IA32_STAR_MSR,
            ((uint64_t)SEL_CS_0 << 32) | ((uint64_t)SEL_CS_3 << 48));
}
...
BOOT_CODE void init_gdt(gdt_entry_t *gdt, tss_t *tss) {
  ...
  gdt[GDT_DS_3] =
      gdt_entry_gdt_data_new(0, 1, 1, 0, 0xf, 1, 3, 1, 0, 0, 0xffff);
...
BOOT_CODE void init_idt_entry(idt_entry_t *idt, interrupt_t interrupt,
                              void (*handler)(void)) {
...
static BOOT_CODE cap_t create_it_pdpt_cap(cap_t vspace_cap, pptr_t pptr,
                                          vptr_t vptr, asid_t asid) {
  cap_t cap;
...
static BOOT_CODE cap_t create_it_pd_cap(cap_t vspace_cap, pptr_t pptr,
                                        vptr_t vptr, asid_t asid) {
  cap_t cap;
...
static BOOT_CODE cap_t create_it_pt_cap(cap_t vspace_cap, pptr_t pptr,
                                        vptr_t vptr, asid_t asid) {
  cap_t cap;
...
BOOT_CODE cap_t create_it_address_space(cap_t root_cnode_cap,
                                        v_region_t it_v_reg) {
  cap_t vspace_cap;
  vptr_t vptr;
  ...
  write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadVSpace),
             vspace_cap);

  /* Create any PDPTs needed for the user land image */
  for (vptr = ROUND_DOWN(it_v_reg.start, PML4_INDEX_OFFSET);
       vptr < it_v_reg.end; vptr += BIT(PML4_INDEX_OFFSET)) {
    if (!provide_cap(
            root_cnode_cap,
  ...
  for (vptr = ROUND_DOWN(it_v_reg.start, PDPT_INDEX_OFFSET);
       vptr < it_v_reg.end; vptr += BIT(PDPT_INDEX_OFFSET)) {
    if (!provide_cap(
            root_cnode_cap,
  ...
  ndks_boot.bi_frame->userImagePaging =
      (seL4_SlotRegion){slot_pos_before, slot_pos_after};
  return vspace_cap;
}
...
static BOOT_CODE cap_t create_it_frame_cap(pptr_t pptr, vptr_t vptr,
                                           asid_t asid, bool_t use_large,
                                           seL4_Word map_type) {
...
BOOT_CODE cap_t create_mapped_it_frame_cap(cap_t vspace_cap, pptr_t pptr,
                                           vptr_t vptr, asid_t asid,
                                           bool_t use_large,
                                           bool_t executable UNUSED) {
  cap_t cap =
      create_it_frame_cap(pptr, vptr, asid, use_large, X86_MappingVSpace);
...
exception_t performASIDPoolInvocation(asid_t asid, asid_pool_t *poolPtr,
  ...
  if (cap_get_capType(vspaceCapSlot->cap) == cap_ept_pml4_cap) {
    ...
    asid_map = asid_map_asid_map_ept_new(
        cap_ept_pml4_cap_get_capPML4BasePtr(vspaceCapSlot->cap));
  } else
#endif
  {
    ...
    asid_map = asid_map_asid_map_vspace_new(
        cap_pml4_cap_get_capPML4BasePtr(vspaceCapSlot->cap));
  }
...
static pdpte_t CONST makeUserPDPTEHugePage(paddr_t paddr,
...
static pdpte_t CONST makeUserPDPTEPageDirectory(paddr_t paddr,
                                                vm_attributes_t vm_attr) {
...
pde_t CONST makeUserPDELargePage(paddr_t paddr, vm_attributes_t vm_attr,
                                 vm_rights_t vm_rights) {
...
pde_t CONST makeUserPDEInvalid(void) {
  /* The bitfield only declares two kinds of PDE entries (page tables or large
   * pages) and an invalid entry should really be a third type, but we can
   * simulate it by creating an invalid (present bit 0) entry of either of the
   * defined types */
...
pte_t CONST makeUserPTE(paddr_t paddr, vm_attributes_t vm_attr,
                        vm_rights_t vm_rights) {
  return pte_new(0,     /* xd                   */
                 paddr, /* page_base_address    */
                 0,     /* global               */
...
static lookupPDPTSlot_ret_t lookupPDPTSlot(vspace_root_t *pml4, vptr_t vptr) {
  ...
  if (!pml4e_ptr_get_present(pml4Slot)) {
    current_lookup_fault =
        lookup_fault_missing_capability_new(PML4_INDEX_OFFSET);
...
lookupPDSlot_ret_t lookupPDSlot(vspace_root_t *pml4, vptr_t vptr) {
  ...
  if ((pdpte_ptr_get_page_size(pdptSlot.pdptSlot) != pdpte_pdpte_pd) ||
    ...
    current_lookup_fault =
        lookup_fault_missing_capability_new(PDPT_INDEX_OFFSET);
    ...
    pd = paddr_to_pptr(
        pdpte_pdpte_pd_ptr_get_pd_base_address(pdptSlot.pdptSlot));
    pdSlot = pd + pdIndex;
...
void unmapPageDirectory(asid_t asid, vptr_t vaddr, pde_t *pd) {
  ...
  if (!(pdpte_ptr_get_page_size(lu_ret.pdptSlot) == pdpte_pdpte_pd &&
        ...
        (pdpte_pdpte_pd_ptr_get_pd_base_address(lu_ret.pdptSlot) ==
         pptr_to_paddr(pd)))) {
    return;
  }
  ...
  *lu_ret.pdptSlot = makeUserPDPTEInvalid();
  ...
  invalidatePageStructureCacheASID(
      pptr_to_paddr(find_ret.vspace_root), asid,
      SMP_TERNARY(tlb_bitmap_get(find_ret.vspace_root), 0));
}

static exception_t performX64PageDirectoryInvocationUnmap(cap_t cap,
                                                          cte_t *ctSlot) {
...
static exception_t performX64PageDirectoryInvocationMap(cap_t cap,
                                                        cte_t *ctSlot,
                                                        pdpte_t pdpte,
                                                        pdpte_t *pdptSlot,
  ...
  *pdptSlot = pdpte;
  invalidatePageStructureCacheASID(
      pptr_to_paddr(vspace), cap_page_directory_cap_get_capPDMappedASID(cap),
      SMP_TERNARY(tlb_bitmap_get(vspace), 0));
  return EXCEPTION_NONE;
}

static exception_t decodeX64PageDirectoryInvocation(word_t label, word_t length,
                                                    cte_t *cte, cap_t cap,
                                                    word_t *buffer) {
  word_t vaddr;
  vm_attributes_t vm_attr;
  ...
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return performX64PageDirectoryInvocationMap(cap, cte, pdpte,
                                              pdptSlot.pdptSlot, vspace);
}
...
static exception_t performX64PDPTInvocationUnmap(cap_t cap, cte_t *ctSlot) {
  if (cap_pdpt_cap_get_capPDPTIsMapped(cap)) {
    ...
    unmapPDPT(cap_pdpt_cap_get_capPDPTMappedASID(cap),
              cap_pdpt_cap_get_capPDPTMappedAddress(cap), pdpt);
...
static exception_t performX64PDPTInvocationMap(cap_t cap, cte_t *ctSlot,
                                               pml4e_t pml4e, pml4e_t *pml4Slot,
                                               vspace_root_t *vspace) {
  ctSlot->cap = cap;
  *pml4Slot = pml4e;
...
static exception_t decodeX64PDPTInvocation(word_t label, word_t length,
                                           cte_t *cte, cap_t cap,
                                           word_t *buffer) {
  word_t vaddr;
...
exception_t decodeX86ModeMMUInvocation(word_t label, word_t length, cptr_t cptr,
                                       cte_t *cte, cap_t cap, bool_t call,
                                       word_t *buffer) {
...
bool_t modeUnmapPage(vm_page_size_t page_size, vspace_root_t *vroot,
                     vptr_t vaddr, void *pptr) {
...
static exception_t performX64ModeMap(cap_t cap, cte_t *ctSlot, pdpte_t pdpte,
                                     pdpte_t *pdptSlot, vspace_root_t *vspace) {
  ctSlot->cap = cap;
  return updatePDPTE(cap_frame_cap_get_capFMappedASID(cap), pdpte, pdptSlot,
                     vspace);
}
...
static create_mapping_pdpte_return_t createSafeMappingEntries_PDPTE(
    paddr_t base, word_t vaddr, vm_rights_t vmRights, vm_attributes_t attr,
    vspace_root_t *vspace) {
...
exception_t decodeX86ModeMapPage(word_t label, vm_page_size_t page_size,
                                 cte_t *cte, cap_t cap, vspace_root_t *vroot,
                                 vptr_t vaddr, paddr_t paddr,
                                 vm_rights_t vm_rights,
                                 vm_attributes_t vm_attr) {
  if (config_set(CONFIG_HUGE_PAGE) && page_size == X64_HugePage) {
    ...
    map_ret =
        createSafeMappingEntries_PDPTE(paddr, vaddr, vm_rights, vm_attr, vroot);
    ...
    switch (label) {
      case X86PageMap:
        return performX64ModeMap(cap, cte, map_ret.pdpte, map_ret.pdptSlot,
                                 vroot);
...
static readWordFromVSpace_ret_t readWordFromVSpace(vspace_root_t *vspace,
                                                   word_t vaddr) {
...
exception_t benchmark_arch_map_logBuffer(word_t frame_cptr) {
  ...
  if (frameSize != X86_LargePage) {
    userError(
        "Invalid size for log Buffer. The kernel expects at least 1M log "
        "buffer");
    current_fault = seL4_Fault_CapFault_new(frame_cptr, false);
...
#endif /* CONFIG_KERNEL_LOG_BUFFER */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/machine/capdl.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
word_t get_tcb_sp(tcb_t *tcb) { return tcb->tcbArch.tcbContext.registers[RSP]; }
...
static void cap_frame_print_attrs_impl(word_t super_user, word_t read_write,
                                       word_t cache_disabled, word_t xd);
...
static void cap_frame_print_attrs_pt(pte_t *ptSlot) {
  cap_frame_print_attrs_impl(
      pte_ptr_get_super_user(ptSlot), pte_ptr_get_read_write(ptSlot),
      pte_ptr_get_cache_disabled(ptSlot), pte_ptr_get_xd(ptSlot));
}
...
static void x86_64_obj_pt_print_slots(pde_t *pdSlot) {
  ...
  for (word_t i = 0; i < BIT(PT_INDEX_OFFSET + PT_INDEX_BITS);
       i += (1UL << PT_INDEX_OFFSET)) {
...
static void x86_64_obj_pd_print_slots(pdpte_t *pdptSlot) {
  ...
  for (word_t i = 0; i < BIT(PD_INDEX_OFFSET + PD_INDEX_BITS);
       i += (1UL << PD_INDEX_OFFSET)) {
    pde_t *pdSlot = pd + GET_PD_INDEX(i);

    if ((pde_ptr_get_page_size(pdSlot) == pde_pde_large) &&
        pde_pde_large_ptr_get_present(pdSlot)) {
...
static void x86_64_obj_pdpt_print_slots(pml4e_t *pml4Slot) {
  ...
  for (word_t i = 0; i < BIT(PDPT_INDEX_OFFSET + PDPT_INDEX_BITS);
       i += (1UL << PDPT_INDEX_OFFSET)) {
...
static void x86_64_obj_pml4_print_slots(pml4e_t *pml4) {
  for (word_t i = 0; i < BIT(PML4_INDEX_OFFSET + PML4_INDEX_BITS);
       i += (1UL << PML4_INDEX_OFFSET)) {
...
void obj_tcb_print_vtable(tcb_t *tcb) {
  ...
  if (isValidVTableRoot(TCB_PTR_CTE_PTR(tcb, tcbVTable)->cap) &&
      !seen(TCB_PTR_CTE_PTR(tcb, tcbVTable)->cap)) {
    pml4e_t *pml4 = PML4E_PTR(
        cap_pml4_cap_get_capPML4BasePtr(TCB_PTR_CTE_PTR(tcb, tcbVTable)->cap));
...
static void _cap_frame_print_attrs_vptr(word_t vptr, vspace_root_t *vspace) {
  ...
  if (pdptSlot.status == EXCEPTION_NONE &&
    ...
    if (pdSlot.status == EXCEPTION_NONE &&
      ...
      assert(ptSlot.status == EXCEPTION_NONE &&
             pte_ptr_get_present(ptSlot.ptSlot));
  ...
  switch (cap_get_capType(cap)) {
    ...
    case cap_page_table_cap: {
      ...
      if (asid) {
        printf("pt_%p_%04lu (asid: %lu)\n",
               ...
               GET_PD_INDEX(vptr), (long unsigned int)asid);
      } else {
        printf("pt_%p_%04lu\n", lookupPDSlot(find_ret.vspace_root, vptr).pdSlot,
               GET_PD_INDEX(vptr));
      }
    ...
    case cap_page_directory_cap: {
      ...
      if (asid) {
        printf("pd_%p_%04lu (asid: %lu)\n",
               lookupPDPTSlot(find_ret.vspace_root, vptr).pdptSlot,
               GET_PDPT_INDEX(vptr), (long unsigned int)asid);
      } else {
        printf("pd_%p_%04lu\n",
               lookupPDPTSlot(find_ret.vspace_root, vptr).pdptSlot,
               GET_PDPT_INDEX(vptr));
    ...
    case cap_pdpt_cap: {
      ...
      if (asid) {
        ...
        printf("pdpt_%p_%04lu\n", lookupPML4Slot(find_ret.vspace_root, vptr),
               GET_PML4_INDEX(vptr));
      }
      break;
    }
...
void print_object_arch(cap_t cap) {
  switch (cap_get_capType(cap)) {
    ...
    case cap_pdpt_cap:
    case cap_pml4_cap:
      /* don't need to deal with these objects since they get handled from
       * vtable */
      break;
...
static void x86_64_cap_pt_print_slots(pde_t *pdSlot, vptr_t vptr) {
  ...
  for (word_t i = 0; i < BIT(PT_INDEX_OFFSET + PT_INDEX_BITS);
    ...
    if (pte_ptr_get_present(ptSlot)) {
      printf("0x%lx: frame_%p_%04lu ", GET_PT_INDEX(i), ptSlot,
             GET_PT_INDEX(i));
...
static void x86_64_cap_pd_print_slots(pdpte_t *pdptSlot, vptr_t vptr) {
  ...
  for (word_t i = 0; i < BIT(PD_INDEX_OFFSET + PD_INDEX_BITS);
    ...
    if ((pde_ptr_get_page_size(pdSlot) == pde_pde_large) &&
        pde_pde_large_ptr_get_present(pdSlot)) {
      printf("0x%lx: frame_%p_%04lu ", GET_PD_INDEX(i), pdSlot,
             GET_PD_INDEX(i));
  ...
  for (word_t i = 0; i < BIT(PD_INDEX_OFFSET + PD_INDEX_BITS);
       i += (1UL << PD_INDEX_OFFSET)) {
    pde_t *pdSlot = pd + GET_PD_INDEX(i);
    if ((pde_ptr_get_page_size(pdSlot) == pde_pde_pt) &&
        pde_pde_pt_ptr_get_present(pdSlot)) {
...
static void x86_64_cap_pdpt_print_slots(pml4e_t *pml4Slot, vptr_t vptr) {
  ...
  for (word_t i = 0; i < BIT(PDPT_INDEX_OFFSET + PDPT_INDEX_BITS);
    ...
    if (pdpte_ptr_get_page_size(pdptSlot) == pdpte_pdpte_1g &&
        pdpte_pdpte_1g_ptr_get_present(pdptSlot)) {
      printf("0x%lx: frame_%p_%04lu ", GET_PDPT_INDEX(i), pdptSlot,
             GET_PDPT_INDEX(i));
      cap_frame_print_attrs_pdpt(pdptSlot);

    } else if (pdpte_pdpte_pd_ptr_get_present(pdptSlot)) {
      printf("0x%lx: pd_%p_%04lu\n", GET_PDPT_INDEX(i), pdptSlot,
             GET_PDPT_INDEX(i));
    }
  }
  printf("}\n"); /* pdpt */
...
#endif /* CONFIG_DEBUG_BUILD */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/machine/registerset.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
const register_t msgRegisters[] = {R10, R8, R9, R15};
compile_assert(consistent_message_registers,
               sizeof(msgRegisters) / sizeof(msgRegisters[0]) ==
                   n_msgRegisters);

const register_t frameRegisters[] = {FaultIP, RSP, FLAGS, RAX, RBX, RCX,
                                     RDX,     RSI, RDI,   RBP, R8,  R9,
                                     R10,     R11, R12,   R13, R14, R15};
compile_assert(consistent_frame_registers,
               sizeof(frameRegisters) / sizeof(frameRegisters[0]) ==
                   n_frameRegisters);

const register_t gpRegisters[] = {FS_BASE, GS_BASE};
compile_assert(consistent_gp_registers,
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/model/smp.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
char nodeSkimScratch[CONFIG_MAX_NUM_NODES][sizeof(nodeInfo_t)] ALIGN(
    L1_CACHE_LINE_SIZE);
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/model/statedata.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#else
pde_t x64KSKernelPDs[BIT(PDPT_INDEX_BITS)][BIT(PD_INDEX_BITS)] ALIGN(
    BIT(seL4_PageDirBits));
#endif
...
word_t x64KSIRQStack[CONFIG_MAX_NUM_NODES][IRQ_STACK_SIZE + 2] ALIGN(
    64) VISIBLE SKIM_BSS;
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/object/objecttype.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
  ...
  switch (cap_get_capType(cap)) {
    case cap_pml4_cap:
      if (final && cap_pml4_cap_get_capPML4IsMapped(cap)) {
        deleteASID(cap_pml4_cap_get_capPML4MappedASID(cap),
...
#ifdef CONFIG_VTX
          case X86_MappingEPT:
            unmapEPTPage(cap_frame_cap_get_capFSize(cap),
...
cap_t Mode_createObject(object_t t, void *regionBase, word_t userSize,
  ...
  switch (t) {
    ...
    case seL4_X86_PDPTObject:
      ...
      return cap_pdpt_cap_new(asidInvalid,        /* capPDPTMappedASID    */
    ...
    case seL4_X64_PML4Object:
      ...
      return cap_pml4_cap_new(asidInvalid,        /* capPML4MappedASID   */
    ...
    case seL4_X86_IOPageTableObject:
      return cap_io_page_table_cap_new(0, 0, 0, asidInvalid,
                                       (word_t)regionBase);
#endif
...
exception_t Mode_decodeInvocation(word_t label, word_t length, cptr_t cptr,
                                  cte_t *slot, cap_t cap, bool_t call,
                                  word_t *buffer) {
  switch (cap_get_capType(cap)) {
    case cap_pml4_cap:
    case cap_pdpt_cap:
    ...
    case cap_page_table_cap:
    case cap_frame_cap:
      return decodeX86MMUInvocation(label, length, cptr, slot, cap, call,
                                    buffer);
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/64/smp/ipi.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/api/faults.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
word_t Arch_setMRs_fault(tcb_t *sender, tcb_t *receiver,
                         word_t *receiveIPCBuffer, word_t faultType) {
  switch (faultType) {
    case seL4_Fault_VMFault: {
...
word_t handleKernelException(word_t vector, word_t errcode, word_t ip,
                             word_t sp, word_t flags, word_t cr0, word_t cr2,
                             word_t cr3, word_t cr4);
...
word_t handleKernelException(word_t vector, word_t errcode, word_t ip,
  ...
  /* Check if we are in a state where we expect a GP fault, if so record it and
   * return */
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/benchmark/benchmark.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/c_traps.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void VISIBLE NORETURN c_handle_interrupt(int irq, int syscall) {
  /* need to run this first as the NODE_LOCK code might end up as a function
   * call with a return, and we need to make sure returns are not exploitable
   * yet on x64 this code ran already */
  if (config_set(CONFIG_ARCH_IA32) &&
      config_set(CONFIG_KERNEL_X86_IBRS_BASIC)) {
    x86_enable_ibrs();
  }
  ...
  if (irq == int_unimpl_dev) {
    ...
    /* Error code is in Error. Pull out bit 5, which is whether it was
     * instruction or data */
    vm_fault_type_t type =
        (NODE_STATE(ksCurThread)->tcbArch.tcbContext.registers[Error] >> 4u) &
        1u;
#ifdef TRACK_KERNEL_ENTRIES
    ...
    ksKernelEntry.word =
        NODE_STATE(ksCurThread)->tcbArch.tcbContext.registers[FaultIP];
#endif
    ...
    handleUserLevelFault(
        irq, NODE_STATE(ksCurThread)->tcbArch.tcbContext.registers[Error]);
...
void NORETURN slowpath(syscall_t syscall) {
  ...
  if (syscall == SysVMEnter && NODE_STATE(ksCurThread)->tcbArch.tcbVCPU) {
    ...
    if (NODE_STATE(ksCurThread)->tcbBoundNotification &&
        notification_ptr_get_state(
            NODE_STATE(ksCurThread)->tcbBoundNotification) ==
            NtfnState_Active) {
      completeSignal(NODE_STATE(ksCurThread)->tcbBoundNotification,
...
void VISIBLE NORETURN c_handle_syscall(word_t cptr, word_t msgInfo,
                                       syscall_t syscall, word_t reply)
#else
void VISIBLE NORETURN c_handle_syscall(word_t cptr, word_t msgInfo,
                                       syscall_t syscall)
#endif
{
  ...
  if (config_set(CONFIG_SYSENTER)) {
    ...
    /* set FaultIP */
    setRegister(NODE_STATE(ksCurThread), FaultIP,
                getRegister(NODE_STATE(ksCurThread), NextIP) - 2);
  }
...
void VISIBLE NORETURN c_handle_vmexit(void) {
  ...
  /* We *always* need to flush the rsb as a guest may have been able to train
   * the rsb with kernel addresses */
  x86_flush_rsb();
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/idle.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/apic.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE bool_t apic_init(bool_t mask_legacy_irqs) {
  ...
  if (!(cpuid & BIT(CPUID_TSC_DEADLINE_BIT))) {
    ...
    printf("Apic Khz %lu, TSC Mhz %lu, ratio %lu\n", (long)apic_khz,
           (long)x86KStscMhz, (long)x86KSapicRatio);
  } else {
  ...
  apic_write_reg(APIC_SVR, apic_svr_new(0,           /* focus_processor_chk */
                                        1,           /* enabled             */
  ...
  apic_write_reg(APIC_LVT_LINT0, apic_lvt_new(0, /* timer_mode      */
  ...
  apic_write_reg(APIC_LVT_LINT1, apic_lvt_new(0, /* timer_mode      */
                                              1, /* masked          */
                                              0, /* trigger_mode    */
                                              0, /* remote_irr      */
                                              0, /* pin_polarity    */
...
#endif
  apic_write_reg(APIC_LVT_TIMER, apic_lvt_new(timer_mode, 0, /* masked */
                                              0,        /* trigger_mode    */
                                              0,        /* remote_irr      */
                                              0,        /* pin_polarity    */
                                              0,        /* delivery_status */
...
void apic_ack_active_interrupt(void) { apic_write_reg(APIC_EOI, 0); }
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/boot.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
/* This function initialises a node's kernel state. It does NOT initialise the
 * CPU. */

BOOT_CODE bool_t init_sys_state(
    cpu_id_t cpu_id, mem_p_regs_t *mem_p_regs, ui_info_t ui_info,
    p_region_t boot_mem_reuse_p_reg,
    /* parameters below not modeled in abstract specification */
    uint32_t num_drhu, paddr_t *drhu_list, acpi_rmrr_list_t *rmrr_list,
    acpi_rsdp_t *acpi_rsdp, seL4_X86_BootInfo_VBE *vbe,
    seL4_X86_BootInfo_mmap_t *mb_mmap, seL4_X86_BootInfo_fb_t *fb_info) {
  ...
  it_v_reg.end =
      ROUND_UP(extra_bi_frame_vptr + BIT(extra_bi_size_bits), PAGE_BITS);
#ifdef CONFIG_IOMMU
  ...
  if (!arch_init_freemem(ui_info.p_reg, it_v_reg, mem_p_regs,
                         extra_bi_size_bits)) {
  ...
  write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapIOPortControl),
  ...
  if (vbe->vbeMode != -1) {
    ...
    memcpy((void *)(rootserver.extra_bi + extra_bi_offset), vbe,
  ...
  if (acpi_rsdp) {
    ...
    *(seL4_BootInfoHeader *)(rootserver.extra_bi + extra_bi_offset) = header;
    ...
    memcpy((void *)(rootserver.extra_bi + extra_bi_offset), acpi_rsdp,
           sizeof(*acpi_rsdp));
  ...
  if (fb_info && fb_info->addr) {
    ...
    *(seL4_BootInfoHeader *)(rootserver.extra_bi + extra_bi_offset) = header;
    extra_bi_offset += sizeof(header);
    memcpy((void *)(rootserver.extra_bi + extra_bi_offset), fb_info,
           sizeof(*fb_info));
  ...
  memcpy((void *)(rootserver.extra_bi + extra_bi_offset), mb_mmap,
         mb_mmap_size);
  ...
  padding_header.len =
      (extra_bi_region.end - extra_bi_region.start) - extra_bi_offset;
  *(seL4_BootInfoHeader *)(extra_bi_region.start + extra_bi_offset) =
      padding_header;
  ...
  extra_bi_ret = create_frames_of_region(
      root_cnode_cap, it_vspace_cap, extra_bi_region, true,
  ...
  ipcbuf_cap =
      create_ipcbuf_frame_cap(root_cnode_cap, it_vspace_cap, ipcbuf_vptr);
  if (cap_get_capType(ipcbuf_cap) == cap_null_cap) {
    return false;
  }

  /* create all userland image frames */
  create_frames_ret = create_frames_of_region(root_cnode_cap, it_vspace_cap,
                                              ui_reg, true, ui_info.pv_offset);
  ...
  tcb_t *initial =
      create_initial_thread(root_cnode_cap, it_vspace_cap, ui_info.v_entry,
  ...
  write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapIOSpace),
             master_iospace_cap());
#else
...
BOOT_CODE bool_t init_cpu(bool_t mask_legacy_irqs) {
  ...
  /* enable the Write Protect bit in cr0. This prevents the kernel from writing
   * to read only memory, which we shouldn't do under correct execution */
  ...
  if (cpuid_007h_ebx_get_smap(ebx_007)) {
    /* if we have user stack trace enabled or dangerous code injection then we
     * cannot enable this as SMAP will make them fault. */
    if (!config_set(CONFIG_PRINTING) &&
        !config_set(CONFIG_DANGEROUS_CODE_INJECTION)) {
      write_cr4(read_cr4() | CR4_SMAP);
    }
  }
  if (cpuid_007h_ebx_get_smep(ebx_007)) {
    /* similar to smap we cannot enable smep if using dangerous code injection.
     * it does not affect stack trace printing though */
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/boot_sys.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE static paddr_t load_boot_module(word_t boot_module_start,
                                          paddr_t load_paddr) {
  v_region_t v_reg;
  word_t entry;
  ...
  printf("size=0x%lx v_entry=%p v_start=%p v_end=%p ", v_reg.end - v_reg.start,
         (void *)entry, (void *)v_reg.start, (void *)v_reg.end);
  ...
  if (v_reg.end + 2 * BIT(PAGE_BITS) > USER_TOP) {
    /* for IPC buffer frame and bootinfo frame, need 2*4K of additional userland
     * virtual memory */
  ...
  /* initialise all initial userland memory and load potentially sparse ELF
   * image */
...
static BOOT_CODE bool_t try_boot_sys_node(cpu_id_t cpu_id) {
  ...
  if (!map_kernel_window(boot_state.num_ioapic, boot_state.ioapic_paddr,
                         boot_state.num_drhu, boot_state.drhu_list)) {
    return false;
  }
  ...
  if (!init_sys_state(
          cpu_id, &boot_state.mem_p_regs, boot_state.ui_info,
          boot_mem_reuse_p_reg,
          /* parameters below not modeled in abstract specification */
          boot_state.num_drhu, boot_state.drhu_list, &boot_state.rmrr_list,
          &boot_state.acpi_rsdp, &boot_state.vbe_info, &boot_state.mb_mmap_info,
          &boot_state.fb_info)) {
    return false;
  }
...
static BOOT_CODE bool_t add_mem_p_regs(p_region_t reg) {
  ...
  if (reg.end > PADDR_TOP && reg.start > PADDR_TOP) {
    /* Return true here as it's not an error for there to exist memory outside
     * the kernel window, we're just going to ignore it and leave it to be given
     * out as device memory */
    return true;
  }
  if (boot_state.mem_p_regs.count == MAX_NUM_FREEMEM_REG) {
    printf(
        "Dropping memory region 0x%lx-0x%lx, try increasing "
        "MAX_NUM_FREEMEM_REG\n",
        reg.start, reg.end);
    return false;
  }
 ...
 */
static BOOT_CODE bool_t parse_mem_map(uint32_t mmap_length,
                                      uint32_t mmap_addr) {
  ...
  while ((word_t)mmap < (word_t)(mmap_addr + mmap_length)) {
    ...
    if (mem_start != (uint64_t)(word_t)mem_start) {
      ...
      if (type == MULTIBOOT_MMAP_USEABLE_TYPE && mem_start >= HIGHMEM_PADDR &&
          mem_length >= BIT(PAGE_BITS)) {
        if (!add_mem_p_regs((p_region_t){
                ROUND_UP(mem_start, PAGE_BITS),
                ROUND_DOWN(mem_start + mem_length, PAGE_BITS),
            })) {
          return false;
        }
      }
...
static BOOT_CODE bool_t try_boot_sys(void) {
  ...
  if (!x86_cpuid_initialize()) {
    printf(
        "Warning: Your x86 CPU has an unsupported vendor, '%s'.\n"
  ...
  if (cpuid_007h_edx_get_ia32_arch_cap_msr(edx)) {
    ...
    if (ia32_arch_capabilities_msr_get_rdcl_no(cap_msr) &&
        config_set(CONFIG_KERNEL_SKIM_WINDOW)) {
      printf(
          "CPU reports not vulnerable to Rogue Data Cache Load (aka Meltdown "
          "https://meltdownattack.com) "
          "yet SKIM window is enabled. Performance is needlessly being "
          "impacted, consider disabling.\n");
    } else if (!ia32_arch_capabilities_msr_get_rdcl_no(cap_msr) &&
               !config_set(CONFIG_KERNEL_SKIM_WINDOW)) {
      printf(
          "CPU reports vulnerable to Rogue Data Cache Load (aka Meltdown "
          "https://meltdownattack.com) "
          "yet SKIM window is *not* enabled. Please re-build with SKIM window "
          "enabled.");
      return false;
    }
  } else {
    ...
    if (config_set(CONFIG_KERNEL_SKIM_WINDOW) &&
        x86_cpuid_get_identity()->vendor == X86_VENDOR_AMD) {
      printf(
          "SKIM window for mitigating Meltdown "
          "(https://www.meltdownattack.com) "
          "not necessary for AMD and performance is being needlessly affected, "
          "consider disabling\n");
    }
    if (!config_set(CONFIG_KERNEL_SKIM_WINDOW) &&
        x86_cpuid_get_identity()->vendor == X86_VENDOR_INTEL) {
      printf(
  ...
  printf("Kernel loaded to: start=0x%lx end=0x%lx size=0x%lx entry=0x%lx\n",
         ...
         boot_state.ki_p_reg.end - boot_state.ki_p_reg.start, (paddr_t)_start);
  ...
  if (!config_set(CONFIG_IOMMU) || cmdline_opt.disable_iommu) {
    ...
    acpi_dmar_scan(&boot_state.acpi_rsdp, boot_state.drhu_list,
                   &boot_state.num_drhu, MAX_NUM_DRHU, &boot_state.rmrr_list);
  }

  /* query available CPUs from ACPI */
  boot_state.num_cpus =
      acpi_madt_scan(&boot_state.acpi_rsdp, boot_state.cpus,
                     &boot_state.num_ioapic, boot_state.ioapic_paddr);
  ...
  printf(
      "Moving loaded userland images to final location: from=0x%lx to=0x%lx "
      "size=0x%lx\n",
      mods_end_paddr, ui_p_regs.start, ui_p_regs.end - ui_p_regs.start);
  memcpy((void *)ui_p_regs.start, (void *)mods_end_paddr,
...
static BOOT_CODE bool_t try_boot_sys_mbi1(multiboot_info_t *mbi) {
  ...
  multiboot_module_t *modules =
      (multiboot_module_t *)(word_t)mbi->part1.mod_list;

  cmdline_parse((const char *)(word_t)mbi->part1.cmdline, &cmdline_opt);

  if ((mbi->part1.flags & MULTIBOOT_INFO_MEM_FLAG) == 0) {
    printf(
        "Boot loader did not provide information about physical memory size\n");
    return false;
  }
  ...
  for (i = 0; i < mbi->part1.mod_count; i++) {
    printf("  module #%ld: start=0x%x end=0x%x size=0x%x name='%s'\n", i,
           modules[i].start, modules[i].end, modules[i].end - modules[i].start,
           (char *)(long)modules[i].name);
    if ((sword_t)(modules[i].end - modules[i].start) <= 0) {
      printf(
          "Invalid boot module size! Possible cause: boot module file not "
          "found by QEMU\n");
      return false;
    }
  ...
  /* initialize the memory. We track two kinds of memory regions. Physical
   * memory that we will use for the kernel, and physical memory regions that we
   * must not give to the user. Memory regions that must not be given to the
   * user include all the physical memory in the kernel window, but also
   * includes any important or kernel devices. */
  ...
  if (mbi->part1.flags & MULTIBOOT_INFO_MMAP_FLAG) {
    ...
    if (multiboot_mmap_length >
        (SEL4_MULTIBOOT_MAX_MMAP_ENTRIES * sizeof(seL4_X86_mb_mmap_t))) {
      printf(
          "Warning: Multiboot has reported more memory map entries, %zd, "
          "than the max amount that will be passed in the bootinfo, %d. "
          "These extra regions will still be turned into untyped caps.",
          multiboot_mmap_length / sizeof(seL4_X86_mb_mmap_t),
          SEL4_MULTIBOOT_MAX_MMAP_ENTRIES);
    ...
    avail.end =
        ROUND_DOWN(avail.start + (mbi->part1.mem_upper << 10), PAGE_BITS);
  ...
  if ((mbi->part1.flags & MULTIBOOT_INFO_GRAPHICS_FLAG) == 0) {
    ...
    boot_state.vbe_info.vbeInfoBlock =
        *(seL4_VBEInfoBlock_t *)(seL4_Word)mbi->part2.vbe_control_info;
    boot_state.vbe_info.vbeModeInfoBlock =
        *(seL4_VBEModeInfoBlock_t *)(seL4_Word)mbi->part2.vbe_mode_info;
...
static BOOT_CODE bool_t try_boot_sys_mbi2(multiboot2_header_t *mbi2) {
  ...
  multiboot2_tag_t const *tag_e =
      (multiboot2_tag_t *)((word_t)mbi2 + mbi2->total_size);
  ...
  while (tag < tag_e && tag->type != MULTIBOOT2_TAG_END) {
    ...
    if (tag->type == MULTIBOOT2_TAG_CMDLINE) {
      ...
      if (ACPI_V1_SIZE == tag->size - sizeof(*tag)) {
        memcpy(&boot_state.acpi_rsdp, (void *)behind_tag,
               tag->size - sizeof(*tag));
      }
    } else if (tag->type == MULTIBOOT2_TAG_ACPI_2) {
      if (sizeof(boot_state.acpi_rsdp) == tag->size - sizeof(*tag)) {
        memcpy(&boot_state.acpi_rsdp, (void *)behind_tag,
               sizeof(boot_state.acpi_rsdp));
      }
    } else if (tag->type == MULTIBOOT2_TAG_MODULE) {
      multiboot2_module_t const *module =
          (multiboot2_module_t const *)behind_tag;
      printf("  module #%d: start=0x%x end=0x%x size=0x%x name='%s'\n",
             mod_count, module->start, module->end, module->end - module->start,
             module->string);
      ...
      multiboot2_memory_t const *e =
          (multiboot2_memory_t *)((word_t)tag + tag->size);
      ...
      for (multiboot2_memory_t const *m = s; m < e; m++) {
        ...
        if (m->type == MULTIBOOT_MMAP_USEABLE_TYPE &&
            m->addr >= HIGHMEM_PADDR) {
      ...
      printf(
          "Got framebuffer info in multiboot2. Current video mode is at "
          "physical address=%llx pitch=%u resolution=%ux%u@%u type=%u\n",
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/cmdline.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
const char cmdline_str_max_num_nodes[] = {'m', 'a', 'x', '_', 'n', 'u', 'm',
                                          '_', 'n', 'o', 'd', 'e', 's', 0};
const char cmdline_str_num_sh_frames[] = {'n', 'u', 'm', '_', 's', 'h', '_',
                                          'f', 'r', 'a', 'm', 'e', 's', 0};
const char cmdline_str_disable_iommu[] = {'d', 'i', 's', 'a', 'b', 'l', 'e',
                                          '_', 'i', 'o', 'm', 'm', 'u', 0};

static int is_space(char c) { return c <= ' '; }

static int UNUSED parse_opt(const char *cmdline, const char *opt, char *value,
                            int bufsize) {
  int len = -1;
  ...
  while (true) {
    ...
    for (optptr = opt; *optptr && *cmdline && (*cmdline != '=') &&
                       !is_space(*cmdline) && (*optptr == *cmdline);
         optptr++, cmdline++)
      ;
    ...
    if (*optptr == '\0' && *cmdline == '=') {
      ...
      for (len = 0; !is_space(*cmdline) && (len < bufsize - 1);
           cmdline++, len++) {
        value[len] = *cmdline;
      }
      if (bufsize) {
...
static int parse_bool(const char *cmdline, const char *opt) {
  ...
  while (1) {
    ...
    for (optptr = opt;
         *optptr && *cmdline && !is_space(*cmdline) && (*optptr == *cmdline);
         optptr++, cmdline++)
      ;
...
static void UNUSED parse_uint16_array(char *str, uint16_t *array,
                                      int array_size) {
  char *last;
  int i = 0;
  int v;
...
void cmdline_parse(const char *cmdline, cmdline_opt_t *cmdline_opt) {
   ...
   * fully standardized and parts are absolete. See
   * http://wiki.osdev.org/Memory_Map_(x86)#BIOS_Data_Area_.28BDA.29 for an
   * explanation
   */
  /*
...
#pragma GCC diagnostic ignored "-Warray-bounds"
  ...
  if (parse_opt(cmdline, "console_port", cmdline_val, MAX_CMDLINE_VAL_LEN) !=
      -1) {
  ...
  /* only start printing here after having parsed/set/initialised the
   * console_port */
  ...
  if (parse_opt(cmdline, "debug_port", cmdline_val, MAX_CMDLINE_VAL_LEN) !=
      -1) {
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/ept.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static lookupEPTPDPTSlot_ret_t CONST lookupEPTPDPTSlot(ept_pml4e_t *pml4,
                                                       vptr_t vptr) {
  ...
  if (!ept_pml4e_ptr_get_read(pml4Slot)) {
    current_lookup_fault =
        lookup_fault_missing_capability_new(EPT_PML4_INDEX_OFFSET);
  ...
  ept_pdpte_t *pdpt =
      paddr_to_pptr(ept_pml4e_ptr_get_pdpt_base_address(pml4Slot));
...
static lookupEPTPDSlot_ret_t lookupEPTPDSlot(ept_pml4e_t *pml4, vptr_t vptr) {
  ...
  if (!ept_pdpte_ptr_get_read(lu_ret.pdptSlot)) {
    current_lookup_fault =
        lookup_fault_missing_capability_new(EPT_PDPT_INDEX_OFFSET);
  ...
  ept_pde_t *pd =
      paddr_to_pptr(ept_pdpte_ptr_get_pd_base_address(lu_ret.pdptSlot));
...
static lookupEPTPTSlot_ret_t lookupEPTPTSlot(ept_pml4e_t *pml4, vptr_t vptr) {
  ...
  if ((ept_pde_ptr_get_page_size(lu_ret.pdSlot) != ept_pde_ept_pde_pt) ||
    ...
    current_lookup_fault =
        lookup_fault_missing_capability_new(EPT_PD_INDEX_OFFSET);
  ...
  ept_pte_t *pt =
      paddr_to_pptr(ept_pde_ept_pde_pt_ptr_get_pt_base_address(lu_ret.pdSlot));
...
static ept_cache_options_t eptCacheFromVmAttr(vm_attributes_t vmAttr) {
  /* Need to sanitise user input, vmAttr will not have been verified at this
   * point. */
  ept_cache_options_t option = vmAttr.words[0];
  if (option != EPTUncacheable && option != EPTWriteCombining &&
      option != EPTWriteThrough && option != EPTWriteBack) {
    option = EPTWriteBack;
  }
  return option;
}
...
static exception_t performEPTPDPTInvocationUnmap(cap_t cap, cte_t *cte) {
  if (cap_ept_pdpt_cap_get_capPDPTIsMapped(cap)) {
    ...
    unmapEPTPDPT(cap_ept_pdpt_cap_get_capPDPTMappedASID(cap),
                 cap_ept_pdpt_cap_get_capPDPTMappedAddress(cap), pdpt);
    clearMemory((void *)pdpt, cap_get_capSizeBits(cap));
  }
...
static exception_t performEPTPDPTInvocationMap(cap_t cap, cte_t *cte,
                                               ept_pml4e_t pml4e,
                                               ept_pml4e_t *pml4Slot,
                                               ept_pml4e_t *pml4) {
  cte->cap = cap;
...
static exception_t decodeX86EPTPDPTInvocation(word_t invLabel, word_t length,
                                              cte_t *cte, cap_t cap,
                                              word_t *buffer) {
  word_t vaddr;
  cap_t pml4Cap;
  ...
  pml4e = ept_pml4e_new(paddr, 1, 1, 1);
...
exception_t decodeX86EPTInvocation(word_t invLabel, word_t length, cptr_t cptr,
...
EPTPageDirectoryMapped_ret_t EPTPageDirectoryMapped(asid_t asid, vptr_t vaddr,
                                                    ept_pde_t *pd) {
  ...
  if (ept_pdpte_ptr_get_read(find_ret.pdptSlot) &&
      ptrFromPAddr(ept_pdpte_ptr_get_pd_base_address(find_ret.pdptSlot)) ==
          pd) {
    ret.pml4 = asid_ret.ept;
...
void unmapEPTPageDirectory(asid_t asid, vptr_t vaddr, ept_pde_t *pd) {
  ...
  if (lu_ret.status == EXCEPTION_NONE) {
    *lu_ret.pdptSlot = ept_pdpte_new(0, /* pd_base_address  */
                                     0, /* avl_cte_depth    */
                                     0, /* execute          */
                                     0, /* write            */
                                     0  /* read             */
...
static exception_t performEPTPDInvocationUnmap(cap_t cap, cte_t *cte) {
  if (cap_ept_pd_cap_get_capPDIsMapped(cap)) {
    ...
    unmapEPTPageDirectory(cap_ept_pd_cap_get_capPDMappedASID(cap),
                          cap_ept_pd_cap_get_capPDMappedAddress(cap), pd);
    clearMemory((void *)pd, cap_get_capSizeBits(cap));
  }
...
static exception_t performEPTPDInvocationMap(cap_t cap, cte_t *cte,
                                             ept_pdpte_t pdpte,
                                             ept_pdpte_t *pdptSlot,
                                             ept_pml4e_t *pml4) {
  cte->cap = cap;
...
exception_t decodeX86EPTPDInvocation(word_t invLabel, word_t length, cte_t *cte,
  ...
  pdpte = ept_pdpte_new(paddr, /* pd_base_address  */
                        0,     /* avl_cte_depth    */
                        1,     /* execute          */
                        1,     /* write            */
                        1      /* read             */
...
EPTPageTableMapped_ret_t EPTPageTableMapped(asid_t asid, vptr_t vaddr,
                                            ept_pte_t *pt) {
  ...
  if (ept_pde_ptr_get_page_size(find_ret.pdSlot) == ept_pde_ept_pde_pt &&
      ptrFromPAddr(
          ept_pde_ept_pde_pt_ptr_get_pt_base_address(find_ret.pdSlot)) == pt) {
    ret.pml4 = asid_ret.ept;
...
void unmapEPTPageTable(asid_t asid, vptr_t vaddr, ept_pte_t *pt) {
  ...
  if (lu_ret.status == EXCEPTION_NONE) {
    *lu_ret.pdSlot = ept_pde_ept_pde_pt_new(0, /* pt_base_address  */
                                            0, /* avl_cte_depth    */
                                            0, /* execute          */
                                            0, /* write            */
                                            0  /* read             */
...
static exception_t performEPTPTInvocationUnmap(cap_t cap, cte_t *cte) {
  if (cap_ept_pt_cap_get_capPTIsMapped(cap)) {
    ...
    unmapEPTPageTable(cap_ept_pt_cap_get_capPTMappedASID(cap),
                      cap_ept_pt_cap_get_capPTMappedAddress(cap), pt);
...
static exception_t performEPTPTInvocationMap(cap_t cap, cte_t *cte,
                                             ept_pde_t pde, ept_pde_t *pdSlot,
                                             ept_pml4e_t *pml4) {
  cte->cap = cap;
  *pdSlot = pde;
  invept(pml4);
...
exception_t decodeX86EPTPTInvocation(word_t invLabel, word_t length, cte_t *cte,
                                     cap_t cap, word_t *buffer) {
  word_t vaddr;
  cap_t pml4Cap;
  ept_pml4e_t *pml4;
  ...
  if (cap_ept_pt_cap_get_capPTIsMapped(cap)) {
    userError(
        "X86EPTPT EPT Page table is already mapped to an EPT page directory.");
    current_syscall_error.type = seL4_InvalidCapability;
    current_syscall_error.invalidCapNumber = 0;
  ...
  pde = ept_pde_ept_pde_pt_new(paddr, /* pt_base_address  */
                               0,     /* avl_cte_depth    */
                               1,     /* execute          */
                               1,     /* write            */
                               1      /* read             */
...
static exception_t performEPTPageMapPTE(cap_t cap, cte_t *cte,
                                        ept_pte_t *ptSlot, ept_pte_t pte,
                                        ept_pml4e_t *pml4) {
  *ptSlot = pte;
  cte->cap = cap;
  invept(pml4);
...
static exception_t performEPTPageMapPDE(cap_t cap, cte_t *cte,
                                        ept_pde_t *pdSlot, ept_pde_t pde1,
                                        ept_pde_t pde2, ept_pml4e_t *pml4) {
  pdSlot[0] = pde1;
...
exception_t decodeX86EPTPageMap(word_t invLabel, word_t length, cte_t *cte,
                                cap_t cap, word_t *buffer) {
  word_t vaddr;
  word_t w_rightsMask;
  paddr_t paddr;
  ...
  switch (frameSize) {
    ...
    case X86_SmallPage: {
      ...
      pte = ept_pte_new(paddr, 0, 0, eptCacheFromVmAttr(vmAttr), 1,
                        WritableFromVMRights(vmRights), 1);
    ...
    case X86_LargePage: {
      ...
      if (LARGE_PAGE_BITS != EPT_PD_INDEX_OFFSET &&
          (ept_pde_ptr_get_page_size(lu_ret.pdSlot + 1) ==
           ept_pde_ept_pde_pt) &&
      ...
      ept_pde_t pde1 =
          ept_pde_ept_pde_2m_new(paddr, 0, 0, eptCacheFromVmAttr(vmAttr), 1,
                                 WritableFromVMRights(vmRights), 1);

      ept_pde_t pde2 = ept_pde_ept_pde_2m_new(
          paddr + BIT(EPT_PD_INDEX_OFFSET), 0, 0, eptCacheFromVmAttr(vmAttr), 1,
          WritableFromVMRights(vmRights), 1);
...
void unmapEPTPage(vm_page_size_t page_size, asid_t asid, vptr_t vptr,
                  void *pptr) {
  findEPTForASID_ret_t find_ret;
  ...
  switch (page_size) {
    ...
    case X86_LargePage: {
      ...
      if (LARGE_PAGE_BITS != EPT_PD_INDEX_OFFSET) {
        assert(ept_pde_ptr_get_page_size(lu_ret.pdSlot + 1) ==
               ept_pde_ept_pde_2m);
        assert(ept_pde_ept_pde_2m_ptr_get_read(lu_ret.pdSlot + 1));
        assert(ept_pde_ept_pde_2m_ptr_get_page_base_address(
                   lu_ret.pdSlot + 1) == addr + BIT(21));
...
#endif /* CONFIG_VTX */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/smp_sys.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_DATA VISIBLE volatile word_t smp_aps_index = 1;
...
BOOT_CODE static void update_logical_id_mappings(void) {
  ...
  for (int i = 0; i < smp_aps_index; i++) {
    if (apic_get_cluster(
            cpu_mapping.index_to_logical_id[getCurrentCPUIndex()]) ==
...
BOOT_CODE void start_boot_aps(void) {
  ...
  while (smp_aps_index < boot_state.num_cpus) {
    ...
    printf("Starting node #%lu with APIC ID %lu \n", current_ap_index,
           boot_state.cpus[current_ap_index]);

    /* update cpu mapping for APs, store APIC ID of the next booting AP
     * as APIC ID are not continoius e.g. 0,2,1,3 for 4 cores with
     * hyperthreading we need to store a mapping to translate the index to real
     * APIC ID */
    cpu_mapping.index_to_cpu_id[current_ap_index] =
        boot_state.cpus[current_ap_index];
...
BOOT_CODE bool_t copy_boot_code_aps(uint32_t mem_lower) {
  ...
  /* Ensure that our boot code fits in the memory hole we want to use, and check
   * this region is free according to multiboot. As boot_cpu_end and
   * boot_cpu_start are link time symbols (and not compile time) this cannot be
  ...
  if (boot_node_top > BOOT_NODE_MAX_PADDR) {
    printf(
        "AP boot code does not fit in chosen memory hole. Can be at most %lu, "
        "is %lu\n",
  ...
  if (mem_lower_bytes < boot_node_top) {
    printf(
        "Need lower physical memory up to %lu to be free. Multiboot reports "
        "only up to %lu\n",
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/thread.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/vspace.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
  ...
  if (x86KSASIDTable[asid_base >> asidLowBits] == pool) {
    for (unsigned int offset = 0; offset < BIT(asidLowBits); offset++) {
      ...
      if (asid_map_get_type(asid_map) == asid_map_asid_map_vspace) {
        vspace_root_t *vspace =
            (vspace_root_t *)asid_map_asid_map_vspace_get_vspace_root(asid_map);
...
exception_t performASIDControlInvocation(void *frame, cte_t *slot,
  ...
  /** AUXUPD: "(True, ptr_retyps 1 (Ptr (ptr_val \<acute>frame) :: asid_pool_C
   * ptr))" */
...
bool_t CONST isValidVTableRoot(cap_t cap) { return isValidNativeRoot(cap); }

BOOT_CODE bool_t map_kernel_window_devices(pte_t *pt, uint32_t num_ioapic,
  ...
  if (!reserve_region((p_region_t){phys, phys + 0x1000})) {
    return false;
  }
  ...
  for (i = 0; i < num_ioapic; i++) {
    ...
    pte = x86_make_device_pte(phys);
    assert(idx ==
           ((PPTR_IOAPIC_START + i * BIT(PAGE_BITS)) & MASK(LARGE_PAGE_BITS)) >>
               PAGE_BITS);
    pt[idx] = pte;
    idx++;
  ...
  for (i = 0; i < num_drhu; i++) {
    ...
    assert(idx ==
           ((PPTR_DRHU_START + i * BIT(PAGE_BITS)) & MASK(LARGE_PAGE_BITS)) >>
               PAGE_BITS);
    pt[idx] = pte;
    idx++;
    if (idx == BIT(PT_INDEX_BITS)) {
...
BOOT_CODE bool_t init_pat_msr(void) {
  ...
  x86_wrmsr(IA32_PAT_MSR,
            ((uint64_t)pat_msr.words[1]) << 32 | pat_msr.words[0]);
  return true;
}
...
lookupPTSlot_ret_t lookupPTSlot(vspace_root_t *vspace, vptr_t vptr) {
  ...
  if ((pde_ptr_get_page_size(pdSlot.pdSlot) != pde_pde_pt) ||
    ...
    current_lookup_fault =
        lookup_fault_missing_capability_new(PAGE_BITS + PT_INDEX_BITS);
    ret.ptSlot = NULL;
    ret.status = EXCEPTION_LOOKUP_FAULT;
    return ret;
  } else {
...
void unmapPage(vm_page_size_t page_size, asid_t asid, vptr_t vptr, void *pptr) {
  ...
  invalidateTranslationSingleASID(
      vptr, asid, SMP_TERNARY(tlb_bitmap_get(find_ret.vspace_root), 0));
}
...
void unmapPageTable(asid_t asid, vptr_t vaddr, pte_t *pt) {
  ...
  if (!(pde_ptr_get_page_size(lu_ret.pdSlot) == pde_pde_pt &&
        ...
        (pde_pde_pt_ptr_get_pt_base_address(lu_ret.pdSlot) ==
         pptr_to_paddr(pt)))) {
    return;
  }
...
static exception_t performX86PageInvocationMapPTE(cap_t cap, cte_t *ctSlot,
                                                  pte_t *ptSlot, pte_t pte,
                                                  vspace_root_t *vspace) {
  ctSlot->cap = cap;
  *ptSlot = pte;
  invalidatePageStructureCacheASID(pptr_to_paddr(vspace),
...
static exception_t performX86PageInvocationMapPDE(cap_t cap, cte_t *ctSlot,
                                                  pde_t *pdSlot, pde_t pde,
                                                  vspace_root_t *vspace) {
  ctSlot->cap = cap;
  *pdSlot = pde;
  invalidatePageStructureCacheASID(pptr_to_paddr(vspace),
...
static exception_t performX86PageInvocationUnmap(cap_t cap, cte_t *ctSlot) {
  ...
  // We have this `if` for something we just asserted to be true for simplicity
  // of verification This has no performance implications as when this function
  // is inlined this `if` will be inside an identical `if` and will therefore be
  // elided
  if (cap_frame_cap_get_capFMappedASID(cap)) {
    unmapPage(cap_frame_cap_get_capFSize(cap),
              cap_frame_cap_get_capFMappedASID(cap),
...
static create_mapping_pte_return_t createSafeMappingEntries_PTE(
    paddr_t base, word_t vaddr, vm_rights_t vmRights, vm_attributes_t attr,
    vspace_root_t *vspace) {
...
static create_mapping_pde_return_t createSafeMappingEntries_PDE(
    paddr_t base, word_t vaddr, vm_rights_t vmRights, vm_attributes_t attr,
    vspace_root_t *vspace) {
...
exception_t decodeX86FrameInvocation(word_t invLabel, word_t length, cte_t *cte,
  ...
  switch (invLabel) {
    case X86PageMap: { /* Map */
      ...
      if (!isValidNativeRoot(vspaceCap)) {
        userError(
            "X86FrameMap: Attempting to map frame into invalid page directory "
            "cap.");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 1;
      ...
      if (cap_frame_cap_get_capFMappedASID(cap) != asidInvalid) {
        ...
        if (cap_frame_cap_get_capFMapType(cap) != X86_MappingVSpace) {
          userError(
              "X86Frame: Attempting to remap frame with different mapping "
              "type");
          current_syscall_error.type = seL4_IllegalOperation;
        ...
        if (cap_frame_cap_get_capFMappedAddress(cap) != vaddr) {
          userError(
              "X86Frame: Attempting to map frame into multiple addresses");
          current_syscall_error.type = seL4_InvalidArgument;
          current_syscall_error.invalidArgumentNumber = 0;
        ...
        /* check vaddr and vtop against USER_TOP to catch case where vaddr +
      ...
      switch (frameSize) {
        ...
        case X86_SmallPage: {
          ...
          map_ret = createSafeMappingEntries_PTE(paddr, vaddr, vmRights, vmAttr,
                                                 vspace);
          if (map_ret.status != EXCEPTION_NONE) {
            return map_ret.status;
          }
          ...
          setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
          return performX86PageInvocationMapPTE(cap, cte, map_ret.ptSlot,
                                                map_ret.pte, vspace);
        }
        ...
        case X86_LargePage: {
          ...
          map_ret = createSafeMappingEntries_PDE(paddr, vaddr, vmRights, vmAttr,
                                                 vspace);
          if (map_ret.status != EXCEPTION_NONE) {
            return map_ret.status;
          }
          ...
          setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
          return performX86PageInvocationMapPDE(cap, cte, map_ret.pdSlot,
                                                map_ret.pde, vspace);
        }

        default: {
          return decodeX86ModeMapPage(invLabel, frameSize, cte, cap, vspace,
                                      vaddr, paddr, vmRights, vmAttr);
        }
      }
...
static exception_t performX86PageTableInvocationUnmap(cap_t cap,
                                                      cte_t *ctSlot) {
...
static exception_t performX86PageTableInvocationMap(cap_t cap, cte_t *ctSlot,
                                                    pde_t pde, pde_t *pdSlot,
                                                    vspace_root_t *root) {
  ctSlot->cap = cap;
  *pdSlot = pde;
  invalidatePageStructureCacheASID(pptr_to_paddr(root),
                                   cap_page_table_cap_get_capPTMappedASID(cap),
                                   SMP_TERNARY(tlb_bitmap_get(root), 0));
  return EXCEPTION_NONE;
}

static exception_t decodeX86PageTableInvocation(word_t invLabel, word_t length,
                                                cte_t *cte, cap_t cap,
                                                word_t *buffer) {
  word_t vaddr;
  vm_attributes_t attr;
  ...
  if (cap_page_table_cap_get_capPTIsMapped(cap)) {
    userError(
        "X86PageTable: Page table is already mapped to a page directory.");
    current_syscall_error.type = seL4_InvalidCapability;
    current_syscall_error.invalidCapNumber = 0;
...
exception_t decodeX86MMUInvocation(word_t invLabel, word_t length, cptr_t cptr,
                                   cte_t *cte, cap_t cap, bool_t call,
                                   word_t *buffer) {
  switch (cap_get_capType(cap)) {
    case cap_frame_cap:
    ...
    case cap_asid_control_cap: {
      ...
      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
      return performASIDControlInvocation(frame, destSlot, parentSlot,
                                          asid_base);
    }
    ...
    case cap_asid_pool_cap: {
      ...
      if (!(isVTableRoot(vspaceCap) ||
            VTX_TERNARY(cap_get_capType(vspaceCap) == cap_ept_pml4_cap, 0)) ||
      ...
      pool =
          x86KSASIDTable[cap_asid_pool_cap_get_capASIDBase(cap) >> asidLowBits];
      if (!pool) {
        current_syscall_error.type = seL4_FailedLookup;
        current_syscall_error.failedLookupWasSource = false;
        current_lookup_fault = lookup_fault_invalid_root_new();
    ...
    default:
      return decodeX86ModeMMUInvocation(invLabel, length, cptr, cte, cap, call,
                                        buffer);
  }
}
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/x2apic.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE void apic_send_init_ipi(cpu_id_t cpu_id) {
  apic_write_icr(x2apic_icr2_new(cpu_id /* dest */
                                 )
                     .words[0],
                 x2apic_icr1_new(0, /* dest_shorthand  */
                                 1, /* trigger_mode    */
                                 1, /* level           */
                                 0, /* dest_mode       */
                                 5, /* delivery_mode   */
                     ...
                     .words[0]);
  apic_write_icr(apic_icr2_new(cpu_id /* dest */
                               )
                     .words[0],
                 x2apic_icr1_new(0, /* dest_shorthand  */
                                 1, /* trigger_mode    */
...
void apic_send_ipi_cluster(irq_t vector, word_t mda) {
  apic_write_icr(x2apic_icr2_new(mda /* message destination address */
                                 )
                     .words[0],
                 x2apic_icr1_new(0,     /* dest_shorthand  */
                                 0,     /* trigger_mode    */
...
#endif /* CONFIG_X2APIC */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/kernel/xapic.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
compile_assert(number_of_cores_invalid_for_logical_ids,
               CONFIG_MAX_NUM_NODES <= 8)

    BOOT_CODE static void init_xapic_ldr(void) {
  uint32_t ldr;
...
BOOT_CODE void apic_send_init_ipi(cpu_id_t cpu_id) {
  apic_write_icr(apic_icr2_new(cpu_id /* dest */
                     ...
                     .words[0],
                 apic_icr1_new(0, /* dest_shorthand  */
                               1, /* trigger_mode    */
                               1, /* level           */
                               0, /* delivery_status */
                               0, /* dest_mode       */
...
void apic_send_ipi_cluster(irq_t vector, word_t mda) {
  ...
  apic_write_icr(apic_icr2_new(mda /* message destination address */
                               )
                     .words[0],
                 apic_icr1_new(0,     /* dest_shorthand  */
                               0,     /* trigger_mode    */
...
#endif /* CONFIG_XAPIC */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/breakpoint.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
bool_t byte8BreakpointsSupported(void) { return byte8_bps_supported; }
...
static void unsetDr7BitsFor(tcb_t *t, uint16_t bp_num) {
  ...
  switch (bp_num) {
    case 0:
      mask = (0x3u << X86_DEBUG_BP0_SIZE_SHIFT) |
             (0x3u << X86_DEBUG_BP0_TYPE_SHIFT);
      break;
    case 1:
      mask = (0x3u << X86_DEBUG_BP1_SIZE_SHIFT) |
             (0x3u << X86_DEBUG_BP1_TYPE_SHIFT);
      break;
    case 2:
      mask = (0x3u << X86_DEBUG_BP2_SIZE_SHIFT) |
             (0x3u << X86_DEBUG_BP2_TYPE_SHIFT);
      break;
    default: /* 3 */
      assert(bp_num == 3);
      mask = (0x3u << X86_DEBUG_BP3_SIZE_SHIFT) |
             (0x3u << X86_DEBUG_BP3_TYPE_SHIFT);
      break;
  }
 ...
 */
PURE static inline word_t convertTypeAndAccessToArch(uint16_t bp_num,
                                                     word_t type, word_t rw) {
  switch (type) {
    ...
    default: /* seL4_DataBreakpoint */
      ...
      type = (rw == seL4_BreakOnWrite) ? X86_DEBUG_BP_TYPE_DATA_WRITE
...
PURE static inline convertedTypeAndAccess_t convertArchToTypeAndAccess(
    word_t dr7, uint16_t bp_num) {
 ...
 */
PURE static inline word_t convertSizeToArch(uint16_t bp_num, word_t type,
                                            word_t size) {
 ...
 */
void setBreakpoint(tcb_t *t, uint16_t bp_num, word_t vaddr, word_t types,
...
static testAndResetSingleStepException_t testAndResetSingleStepException(
    tcb_t *t) {
...
exception_t handleUserLevelDebugException(int int_vector) {
  ...
  if (int_vector == int_software_break_request) {
    current_fault = seL4_Fault_DebugException_new(
        getRestartPC(NODE_STATE(ksCurThread)), 0, seL4_SoftwareBreakRequest);
  } else {
    /* Hardware breakpoint trigger is detected using DR6 */
    active_bp = getAndResetActiveBreakpoint(ct);
    if (active_bp.bp_num >= 0) {
      current_fault = seL4_Fault_DebugException_new(
          active_bp.vaddr, active_bp.bp_num, active_bp.reason);
    } else {
      ...
      if (single_step_info.ret == true) {
        ...
        current_fault = seL4_Fault_DebugException_new(
            single_step_info.instr_vaddr, 0, seL4_SingleStep);
      } else {
        return EXCEPTION_SYSCALL_ERROR;
      }
    }
...
void Arch_initBreakpointContext(user_breakpoint_state_t *uds) {
  ...
  uds->dr[4] = readDr6Reg() & ~(BIT(0) | BIT(1) | BIT(2) | BIT(3) |
                                X86_DEBUG_DR6_SINGLE_STEP_FLAG);

  uds->dr[5] =
      readDr7Reg() & ~(X86_DEBUG_BP0_ENABLE_BIT | X86_DEBUG_BP1_ENABLE_BIT |
                       X86_DEBUG_BP2_ENABLE_BIT | X86_DEBUG_BP3_ENABLE_BIT);
}

#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/capdl.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/cpu_identification.c"
/*
...
const char X86_CPUID_VENDOR_STRING_INTEL[] = {'G', 'e', 'n', 'u', 'i', 'n', 'e',
                                              'I', 'n', 't', 'e', 'l', 0};
const char X86_CPUID_VENDOR_STRING_AMD_LEGACY[] = {
    'A', 'M', 'D', 'i', 's', 'b', 'e', 't', 't', 'e', 'r', '!', 0};
const char X86_CPUID_VENDOR_STRING_AMD[] = {'A', 'u', 't', 'h', 'e', 'n', 't',
                                            'i', 'c', 'A', 'M', 'D', 0};

BOOT_BSS static cpu_identity_t cpu_identity;

BOOT_CODE cpu_identity_t *x86_cpuid_get_identity(void) { return &cpu_identity; }
...
BOOT_CODE static void x86_cpuid_intel_identity_initialize(
    cpu_identity_t *ci, struct family_model original) {
...
BOOT_CODE static void x86_cpuid_amd_identity_initialize(
    cpu_identity_t *ci, struct family_model original) {
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/fpu.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE bool_t Arch_initFpu(void) {
  ...
  write_cr0((read_cr0() & ~CR0_EMULATION) | CR0_MONITOR_COPROC |
            CR0_NUMERIC_ERROR);
  enableFpu();
  ...
  if (config_set(CONFIG_XSAVE)) {
    ...
    uint64_t desired_features =
        config_ternary(CONFIG_XSAVE, CONFIG_XSAVE_FEATURE_SET, 1);
    ...
    /* check feature mask */
    xsave_features =
        ((uint64_t)x86_cpuid_edx(0x0d, 0x0) << 32) | x86_cpuid_eax(0x0d, 0x0);
    ...
    if (x86_cpuid_ebx(0x0d, 0x0) < CONFIG_XSAVE_SIZE) {
      printf(
          "XSAVE buffer set set to %d, but only needs to be %d.\n"
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/hardware.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE void init_sysenter_msrs(void) {
  ...
  if (config_set(CONFIG_ARCH_IA32) && !config_set(CONFIG_HARDWARE_DEBUG_API)) {
     ...
     * sysenter esp */
    x86_wrmsr(IA32_SYSENTER_ESP_MSR,
              (uint64_t)(word_t)((char *)&x86KSGlobalState[CURRENT_CPU_INDEX()]
                                     .x86KStss.tss.words[0] +
                                 4));
  }
}

word_t PURE getRestartPC(tcb_t *thread) { return getRegister(thread, FaultIP); }

void setNextPC(tcb_t *thread, word_t v) { setRegister(thread, NextIP, v); }
...
void flushCacheRange(void *vaddr, uint32_t size_bits) {
  ...
  for (v = ROUND_DOWN((word_t)vaddr, x86KScacheLineSizeBits);
       v < (word_t)vaddr + BIT(size_bits); v += BIT(x86KScacheLineSizeBits)) {
...
BOOT_CODE bool_t disablePrefetchers(void) {
  ...
  uint32_t valid_models[] = {
      BROADWELL_1_MODEL_ID,    BROADWELL_2_MODEL_ID,    BROADWELL_3_MODEL_ID,
      BROADWELL_4_MODEL_ID,    BROADWELL_5_MODEL_ID,    HASWELL_1_MODEL_ID,
      HASWELL_2_MODEL_ID,      HASWELL_3_MODEL_ID,      HASWELL_4_MODEL_ID,
      IVY_BRIDGE_1_MODEL_ID,   IVY_BRIDGE_2_MODEL_ID,   IVY_BRIDGE_3_MODEL_ID,
      SANDY_BRIDGE_1_MODEL_ID, SANDY_BRIDGE_2_MODEL_ID, WESTMERE_1_MODEL_ID,
      WESTMERE_2_MODEL_ID,     WESTMERE_3_MODEL_ID,     NEHALEM_1_MODEL_ID,
      NEHALEM_2_MODEL_ID,      NEHALEM_3_MODEL_ID,      SKYLAKE_1_MODEL_ID,
      SKYLAKE_2_MODEL_ID};
...
BOOT_CODE void enablePMCUser(void) { write_cr4(read_cr4() | CR4_PCE); }
...
BOOT_CODE bool_t init_ibrs(void) {
  ...
  if ((config_set(CONFIG_KERNEL_X86_IBRS_BASIC) ||
       config_set(CONFIG_KERNEL_X86_IBRS_STIBP)) &&
      !support_ibrs) {
    printf("IBRS not supported by CPU\n");
    return false;
  } else if (support_ibrs) {
    /* 'disable' IBRS. For IBRS_BASIC this does nothing, and for STIBP this will
     * cause us to enable STIBP, and we can then forget about it */
    x86_disable_ibrs();
  }
  ...
  if (config_set(CONFIG_KERNEL_X86_IBRS_BASIC) && enhanced_ibrs) {
    printf(
        "Kernel configured for basic IBRS, but CPU supports enhanced IBRS. "
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/machine/registerset.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/model/statedata.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
x86_arch_global_state_t x86KSGlobalState[CONFIG_MAX_NUM_NODES] ALIGN(
    L1_CACHE_LINE_SIZE) SKIM_BSS;
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/interrupt.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
exception_t Arch_checkIRQ(word_t irq_w) {
  if (config_set(CONFIG_IRQ_PIC) && irq_w >= irq_isa_min &&
      irq_w <= irq_isa_max) {
    return EXCEPTION_NONE;
  }
  if (config_set(CONFIG_IRQ_IOAPIC)) {
    ...
    userError("IRQControl: IRQ %ld should be in range %ld - %ld", irq_w,
              (long)irq_isa_min, (long)irq_isa_max);
...
static exception_t Arch_invokeIRQControl(irq_t irq, cte_t *handlerSlot,
                                         cte_t *controlSlot,
...
static exception_t invokeIssueIRQHandlerIOAPIC(irq_t irq, word_t ioapic,
                                               word_t pin, word_t level,
                                               word_t polarity, word_t vector,
                                               cte_t *handlerSlot,
                                               cte_t *controlSlot) {
  x86_irq_state_t irqState =
      x86_irq_state_irq_ioapic_new(ioapic, pin, level, polarity, 1);
...
exception_t Arch_decodeIRQControlInvocation(word_t invLabel, word_t length,
  ...
  if (invLabel != X86IRQIssueIRQHandlerIOAPIC &&
  ...
  if (irq > irq_user_max - irq_user_min) {
    userError("IRQControl: Invalid irq %ld should be between 0-%ld", (long)irq,
              (long)(irq_user_max - irq_user_min));
    current_syscall_error.type = seL4_RangeError;
  ...
  switch (invLabel) {
    case X86IRQIssueIRQHandlerIOAPIC: {
      ...
      status =
          ioapic_decode_map_pin_to_vector(ioapic, pin, level, polarity, vector);
      if (status != EXCEPTION_NONE) {
        return status;
      }

      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
      return invokeIssueIRQHandlerIOAPIC(irq, ioapic, pin, level, polarity,
                                         vector, destSlot, srcSlot);
    } break;
    ...
    default:
      /* the check at the start of this function should guarantee we do not get
       * here */
      fail("IRQControl: Illegal operation");
  }
}
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/ioport.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline void apply_pattern(word_t_may_alias *w, word_t pattern,
                                 bool_t set) {
  if (set) {
    *w |= pattern;
  } else {
...
static exception_t ensurePortOperationAllowed(cap_t cap, uint32_t start_port,
                                              uint32_t size) {
  ...
  if ((start_port < first_allowed) || (end_port > last_allowed)) {
    userError("IOPort: Ports %d--%d fall outside permitted range %d--%d.",
              (int)start_port, (int)end_port, (int)first_allowed,
              (int)last_allowed);
    current_syscall_error.type = seL4_IllegalOperation;
    return EXCEPTION_SYSCALL_ERROR;
  }
...
static bool_t isIOPortRangeFree(uint16_t first_port, uint16_t last_port) {
  ...
  if (low_word == high_word) {
    if ((x86KSAllocatedIOPorts[low_word] &
         make_pattern(low_index, high_index + 1)) != 0) {
      return false;
    }
    return true;
  }
  ...
  if ((x86KSAllocatedIOPorts[low_word] &
       make_pattern(low_index, CONFIG_WORD_SIZE)) != 0) {
    return false;
  }
  low_word++;
  ...
  if ((x86KSAllocatedIOPorts[low_word] & make_pattern(0, high_index + 1)) !=
      0) {
    return false;
  }
  return true;
...
static exception_t invokeX86PortControl(uint16_t first_port, uint16_t last_port,
                                        cte_t *ioportSlot, cte_t *controlSlot) {
...
exception_t decodeX86PortControlInvocation(word_t invLabel, word_t length,
  ...
  lu_ret = lookupTargetSlot(cnodeCap, index, depth);
  if (lu_ret.status != EXCEPTION_NONE) {
    userError("Target slot for new IO Port cap invalid: cap %lu.",
  ...
  status = ensureEmptySlot(destSlot);
  if (status != EXCEPTION_NONE) {
    userError("Target slot for new IO Port cap not empty: cap %lu.",
              getExtraCPtr(buffer, 0));
    return status;
  }
...
static exception_t invokeX86PortIn(word_t invLabel, uint16_t port,
                                   bool_t call) {
  uint32_t res;
  tcb_t *thread;
  thread = NODE_STATE(ksCurThread);
...
static exception_t invokeX86PortOut(word_t invLabel, uint16_t port,
                                    uint32_t data) {
  switch (invLabel) {
    case X86IOPortOut8:
      out8(port, data);
...
exception_t decodeX86PortInvocation(word_t invLabel, word_t length, cptr_t cptr,
                                    cte_t *slot, cap_t cap, bool_t call,
                                    word_t *buffer) {
  exception_t ret;

  if (invLabel == X86IOPortIn8 || invLabel == X86IOPortIn16 ||
      invLabel == X86IOPortIn32) {
    if (length < 1) {
  ...
  } else if (invLabel == X86IOPortOut8 || invLabel == X86IOPortOut16 ||
    ...
    /* We construct the value for data from raw_data based on the actual size of
       the port operation. This ensures that there is no 'random' user data left
       over in the value passed to invokeX86PortOut. Whilst invokeX86PortOut
       will ignore any extra data and cast down to the correct word size
       removing the extra here is currently relied upon for verification */
    uint32_t data;
...
void setIOPortMask(void *ioport_bitmap, uint16_t low, uint16_t high,
                   bool_t set) {
  ...
  if (low_word == high_word) {
    apply_pattern(bitmap + low_word, make_pattern(low_index, high_index + 1),
                  set);
  } else {
    // operate on the potentially partial first word
    apply_pattern(bitmap + low_word, make_pattern(low_index, CONFIG_WORD_SIZE),
                  set);
    low_word++;
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/iospace.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE cap_t master_iospace_cap(void) {
  ...
  return cap_io_space_cap_new(0, /* capDomainID  */
                              0  /* capPCIDevice */
  );
}
...
static vtd_cte_t *lookup_vtd_context_slot(cap_t cap) {
  ...
  vtd_context_index =
      (get_pci_dev(pci_request_id) << 3) | get_pci_fun(pci_request_id);
  vtd_context_slot = &vtd_context[vtd_context_index];

  return vtd_context_slot;
}

static lookupIOPTSlot_ret_t lookupIOPTSlot_resolve_levels(
    vtd_pte_t *iopt, word_t translation, word_t levels_to_resolve,
    word_t levels_remaining) {
  ...
  iopt_index =
      (translation >>
       (VTD_PT_INDEX_BITS *
        (x86KSnumIOPTLevels - 1 - (levels_to_resolve - levels_remaining)))) &
  ...
  return lookupIOPTSlot_resolve_levels(next_iopt_slot, translation,
                                       levels_to_resolve, levels_remaining - 1);
}

static inline lookupIOPTSlot_ret_t lookupIOPTSlot(vtd_pte_t *iopt,
                                                  word_t io_address) {
  lookupIOPTSlot_ret_t ret;
...
void unmapVTDContextEntry(cap_t cap) {
  ...
  assert(cte != 0);
  *cte = vtd_cte_new(0, false, 0, 0, 0, false);
...
static exception_t performX86IOPTInvocationMapContextRoot(
    cap_t cap, cte_t *ctSlot, vtd_cte_t vtd_cte, vtd_cte_t *vtd_context_slot) {
...
static exception_t performX86IOPTInvocationMapPT(cap_t cap, cte_t *ctSlot,
                                                 vtd_pte_t iopte,
                                                 vtd_pte_t *ioptSlot) {
  *ioptSlot = iopte;
...
exception_t decodeX86IOPTInvocation(word_t invLabel, word_t length, cte_t *slot,
                                    cap_t cap, word_t *buffer) {
  cap_t io_space;
  paddr_t paddr;
  uint32_t pci_request_id;
  ...
  io_address =
      getSyscallArg(0, buffer) & ~MASK(VTD_PT_INDEX_BITS + seL4_PageBits);
  ...
  paddr =
      pptr_to_paddr(VTD_PTE_PTR(cap_io_page_table_cap_get_capIOPTBasePtr(cap)));
  vtd_context_slot = lookup_vtd_context_slot(io_space);

  if (!vtd_cte_ptr_get_present(vtd_context_slot)) {
    /* 1st Level Page Table */
    vtd_cte_t vtd_cte =
        vtd_cte_new(domain_id,              /* domain ID                   */
                    false,                  /* RMRR                        */
    ...
    setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
    return performX86IOPTInvocationMapContextRoot(cap, slot, vtd_cte,
                                                  vtd_context_slot);
  } else {
    ...
    iopte = vtd_pte_new(paddr, /* physical addr            */
...
static exception_t performX86IOInvocationMap(cap_t cap, cte_t *ctSlot,
                                             vtd_pte_t iopte,
                                             vtd_pte_t *ioptSlot) {
  ctSlot->cap = cap;
  *ioptSlot = iopte;
...
exception_t decodeX86IOMapInvocation(word_t length, cte_t *slot, cap_t cap,
                                     word_t *buffer) {
  cap_t io_space;
  word_t io_address;
  uint32_t pci_request_id;
  ...
  bool_t write = seL4_CapRights_get_capAllowWrite(dma_cap_rights_mask) &&
                 (frame_cap_rights == VMReadWrite);
  bool_t read = seL4_CapRights_get_capAllowRead(dma_cap_rights_mask) &&
                (frame_cap_rights != VMKernelOnly);
  if (write || read) {
...
void deleteIOPageTable(cap_t io_pt_cap) {
  ...
  if (cap_io_page_table_cap_get_capIOPTIsMapped(io_pt_cap)) {
    ...
    if (level == 0) {
      ...
      if (pptr_to_paddr(vtd_pte) !=
          pptr_to_paddr(
              (void *)cap_io_page_table_cap_get_capIOPTBasePtr(io_pt_cap))) {
        return;
      }
      *vtd_context_slot = vtd_cte_new(0,     /* Domain ID          */
                                      false, /* RMRR               */
                                      0,     /* Address Width      */
      ...
      lu_ret = lookupIOPTSlot_resolve_levels(vtd_pte, io_address >> PAGE_BITS,
                                             level - 1, level - 1);
      ...
      if (vtd_pte_ptr_get_addr(lu_ret.ioptSlot) !=
          pptr_to_paddr(
              (void *)cap_io_page_table_cap_get_capIOPTBasePtr(io_pt_cap))) {
        return;
      }
      *lu_ret.ioptSlot = vtd_pte_new(0, /* Physical Address */
                                     0, /* Read Permission  */
                                     0  /* Write Permission */
      );
...
void unmapIOPage(cap_t cap) {
  ...
  if (vtd_pte_ptr_get_addr(lu_ret.ioptSlot) !=
      pptr_to_paddr((void *)cap_frame_cap_get_capFBasePtr(cap))) {
    return;
  }
...
#endif /* CONFIG_IOMMU */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/objecttype.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
cap_t CONST Arch_updateCapData(bool_t preserve, word_t data, cap_t cap) {
  /* Avoid a switch statement with just a 'default' case as the C parser does
   * not like this */
#ifdef CONFIG_IOMMU
  switch (cap_get_capType(cap)) {
    case cap_io_space_cap: {
      ...
      if (!preserve && cap_io_space_cap_get_capPCIDevice(cap) == 0 &&
          domainID >= x86KSFirstValidIODomain && domainID != 0 &&
...
cap_t Arch_createObject(object_t t, void *regionBase, word_t userSize,
  ...
  switch (t) {
    ...
    case seL4_X86_EPTPML4Object:
      return cap_ept_pml4_cap_new(0,                 /* capPML4IsMapped      */
                                  VPID_INVALID,      /* capPML4MappedASID    */
                                  (word_t)regionBase /* capPML4BasePtr       */
      );
    case seL4_X86_EPTPDPTObject:
      return cap_ept_pdpt_cap_new(0,                 /* capPDPTMappedAddress */
                                  0,                 /* capPDPTIsMapped      */
                                  VPID_INVALID,      /* capPDPTMappedASID    */
                                  (word_t)regionBase /* capPDPTBasePtr      */
      );
    case seL4_X86_EPTPDObject:
      return cap_ept_pd_cap_new(0,                 /* capPDMappedAddress   */
                                0,                 /* capPDIsMapped        */
                                VPID_INVALID,      /* capPDMappedASID      */
                                (word_t)regionBase /* capPDBasePtr         */
      );
    case seL4_X86_EPTPTObject:
      return cap_ept_pt_cap_new(0,                 /* capPTMappedAddress   */
                                0,                 /* capPTIsMapped        */
...
exception_t Arch_decodeInvocation(word_t invLabel, word_t length, cptr_t cptr,
  ...
  switch (cap_get_capType(cap)) {
    case cap_asid_control_cap:
    case cap_asid_pool_cap:
      return decodeX86MMUInvocation(invLabel, length, cptr, slot, cap, call,
                                    buffer);
    case cap_io_port_control_cap:
      return decodeX86PortControlInvocation(invLabel, length, cptr, slot, cap,
                                            buffer);
    case cap_io_port_cap:
      return decodeX86PortInvocation(invLabel, length, cptr, slot, cap, call,
                                     buffer);
#ifdef CONFIG_IOMMU
    case cap_io_space_cap:
...
#ifdef CONFIG_VTX
    case cap_vcpu_cap:
      return decodeX86VCPUInvocation(invLabel, length, cptr, slot, cap, call,
                                     buffer);
    case cap_ept_pml4_cap:
    case cap_ept_pdpt_cap:
    case cap_ept_pd_cap:
...
#endif
    default:
      return Mode_decodeInvocation(invLabel, length, cptr, slot, cap, call,
                                   buffer);
  }
}
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/tcb.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
word_t CONST Arch_decodeTransfer(word_t flags) { return 0; }
...
exception_t decodeSetEPTRoot(cap_t cap) {
  ...
  return performSetEPTRoot(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)),
                           dc_ret.cap, rootSlot);
}
#endif
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/object/vcpu.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
const vcpu_gp_register_t crExitRegs[] = {VCPU_EAX, VCPU_ECX, VCPU_EDX,
                                         VCPU_EBX, VCPU_ESP, VCPU_EBP,
                                         VCPU_ESI, VCPU_EDI};
...
static void vmclear(void *vmcs_ptr) {
  ...
  asm volatile("vmclear %0" : : "m"(physical_address) : "cc");
}
...
static void switchVCPU(vcpu_t *vcpu) {
  ...
  if (vcpu->last_cpu != getCurrentCPUIndex() &&
      ARCH_NODE_STATE_ON_CORE(x86KSCurrentVCPU, vcpu->last_cpu) == vcpu) {
  ...
  if (vcpu->last_cpu != getCurrentCPUIndex()) {
    /* migrate host state */
    vmwrite(VMX_HOST_TR_BASE,
            (word_t)&x86KSGlobalState[CURRENT_CPU_INDEX()].x86KStss);
    vmwrite(VMX_HOST_GDTR_BASE,
            (word_t)x86KSGlobalState[CURRENT_CPU_INDEX()].x86KSgdt);
    vmwrite(VMX_HOST_IDTR_BASE,
            (word_t)x86KSGlobalState[CURRENT_CPU_INDEX()].x86KSidt);
    vmwrite(VMX_HOST_SYSENTER_ESP,
            (uint64_t)(word_t)((char *)&x86KSGlobalState[CURRENT_CPU_INDEX()]
                                   .x86KStss.tss.words[0] +
                               4));
  }
...
static bool_t BOOT_CODE init_vtx_fixed_values(bool_t useTrueMsrs) {
  uint32_t pin_control_mask = BIT(0) |       // Extern interrupt exiting
                              BIT(3) |       // NMI exiting
                              BIT(5);        // virtual NMIs
  uint32_t primary_control_mask = BIT(25) |  // Use I/O bitmaps
                                  BIT(28) |  // Use MSR bitmaps
                                  BIT(31);   // Activate secondary controls
  uint32_t secondary_control_mask = BIT(1);  // Enable EPT
  uint32_t exit_control_mask = BIT(2) |      // Save debug controls
  ...
  if (!(exit_control_low & BIT(12))) {
    ...
    printf(
        "vt-x: Load IA32_PERF_GLOBAL_CONTROL not supported. Hardware debugging "
        "may not work\n");
  } else {
  ...
  if (!(exit_control_low & BIT(15))) {
    ...
    printf(
        "vt-x: Interrupt ack on exit not supported. Expect performance "
        "degredation\n");
  } else {
...
static bool_t BOOT_CODE check_vtx_fixed_values(bool_t useTrueMsrs) {
  ...
  uint32_t local_secondary_control_high =
      x86_rdmsr_low(IA32_VMX_PROCBASED_CTLS2_MSR);
  uint32_t local_secondary_control_low =
      x86_rdmsr_high(IA32_VMX_PROCBASED_CTLS2_MSR);
   ...
   * '~local_low & high == 0'
   */
  return (local_pin_control_high & pin_control_high) ==
             local_pin_control_high &&
         (~local_pin_control_low & pin_control_high) == 0 &&
         (local_primary_control_high & primary_control_high) ==
             local_primary_control_high &&
         (~local_primary_control_low & primary_control_high) == 0 &&
         (local_secondary_control_high & secondary_control_high) ==
             local_secondary_control_high &&
         (~local_secondary_control_low & secondary_control_high) == 0 &&
         (local_exit_control_high & exit_control_high) ==
             local_exit_control_high &&
         (~local_exit_control_low & exit_control_high) == 0 &&
         (local_entry_control_high & entry_control_high) ==
             local_entry_control_high &&
         (~local_entry_control_low & entry_control_high) == 0 &&
         local_cr0_high == cr0_high && local_cr0_low == cr0_low &&
         local_cr4_high == cr4_high && local_cr4_low == cr4_low;
}

static inline uint32_t applyFixedBits(uint32_t original, uint32_t high,
                                      uint32_t low) {
  original |= high;
  original &= low;
  return original;
...
void vcpu_init(vcpu_t *vcpu) {
   ...
   * the kernel address space, so we set it here instead of lazily in
   * restoreVMCS */
  ...
  vmwrite(VMX_CONTROL_PIN_EXECUTION_CONTROLS,
          pin_control_high & pin_control_low);
  vmwrite(VMX_CONTROL_PRIMARY_PROCESSOR_CONTROLS,
          primary_control_high & primary_control_low);
  vmwrite(VMX_CONTROL_SECONDARY_PROCESSOR_CONTROLS,
          secondary_control_high & secondary_control_low);
  ...
  vmwrite(VMX_CONTROL_IOB_ADDRESS,
          pptr_to_paddr((char *)vcpu->io + (VCPU_IOBITMAP_SIZE / 2)));
}
...
static exception_t decodeVCPUWriteRegisters(cap_t cap, word_t length,
                                            word_t *buffer) {
  if (length < 7) {
  ...
  return invokeVCPUWriteRegisters(VCPU_PTR(cap_vcpu_cap_get_capVCPUPtr(cap)),
                                  buffer);
}

static exception_t invokeEnableIOPort(vcpu_t *vcpu, cte_t *slot, cap_t cap,
...
static exception_t decodeEnableIOPort(cap_t cap, word_t length,
                                      word_t *buffer) {
  vcpu_t *vcpu;
  cap_t ioCap;
  cte_t *ioSlot;
  ...
  if (low < cap_io_port_cap_get_capIOPortFirstPort(ioCap) ||
      high > cap_io_port_cap_get_capIOPortLastPort(ioCap)) {
    userError(
        "VCPU EnableIOPort: Requested range not valid for given IOPort cap");
    current_syscall_error.type = seL4_InvalidArgument;
    current_syscall_error.invalidArgumentNumber = 0;
    return EXCEPTION_SYSCALL_ERROR;
  }
...
static exception_t invokeDisableIOPort(vcpu_t *vcpu, uint16_t low,
                                       uint16_t high) {
...
static exception_t decodeDisableIOPort(cap_t cap, word_t length,
                                       word_t *buffer) {
  vcpu_t *vcpu;
  uint16_t low, high;
...
static exception_t invokeWriteVMCS(vcpu_t *vcpu, bool_t call, word_t *buffer,
                                   word_t field, word_t value) {
  tcb_t *thread;
  thread = NODE_STATE(ksCurThread);
...
static exception_t decodeWriteVMCS(cap_t cap, word_t length, bool_t call,
                                   word_t *buffer) {
  word_t field;
  word_t value;
  ...
  switch (field) {
    ...
    case VMX_CONTROL_SECONDARY_PROCESSOR_CONTROLS:
      value =
          applyFixedBits(value, secondary_control_high, secondary_control_low);
      break;
  ...
  return invokeWriteVMCS(VCPU_PTR(cap_vcpu_cap_get_capVCPUPtr(cap)), call,
                         buffer, field, value);
}
...
static exception_t invokeReadVMCS(vcpu_t *vcpu, word_t field, bool_t call,
                                  word_t *buffer) {
  tcb_t *thread;
  thread = NODE_STATE(ksCurThread);
...
static exception_t decodeReadVMCS(cap_t cap, word_t length, bool_t call,
                                  word_t *buffer) {
  if (length < 1) {
  ...
  return invokeReadVMCS(VCPU_PTR(cap_vcpu_cap_get_capVCPUPtr(cap)), field, call,
                        buffer);
}
...
static exception_t decodeSetTCB(cap_t cap, word_t length, word_t *buffer) {
  ...
  return invokeSetTCB(VCPU_PTR(cap_vcpu_cap_get_capVCPUPtr(cap)),
                      TCB_PTR(cap_thread_cap_get_capTCBPtr(tcbCap)));
}
...
void vcpu_update_state_sysvmenter(vcpu_t *vcpu) {
  ...
  vmwrite(VMX_CONTROL_PRIMARY_PROCESSOR_CONTROLS,
          applyFixedBits(getSyscallArg(1, buffer), primary_control_high,
                         primary_control_low));
...
void vcpu_sysvmenter_reply_to_user(tcb_t *tcb) {
  ...
  setMR(tcb, buffer, SEL4_VMENTER_CALL_CONTROL_ENTRY_MR,
        vmread(VMX_CONTROL_ENTRY_INTERRUPTION_INFO));
  setRegister(tcb, msgInfoRegister, 0);
}

exception_t decodeX86VCPUInvocation(word_t invLabel, word_t length, cptr_t cptr,
                                    cte_t *slot, cap_t cap, bool_t call,
                                    word_t *buffer) {
  switch (invLabel) {
    case X86VCPUSetTCB:
...
BOOT_CODE bool_t vtx_init(void) {
  ...
  if (!feature_control_msr_get_vmx_outside_smx(feature_control)) {
    ...
    feature_control =
        feature_control_msr_set_vmx_outside_smx(feature_control, 1);
    x86_wrmsr_parts(IA32_FEATURE_CONTROL_MSR,
                    x86_rdmsr_high(IA32_FEATURE_CONTROL_MSR),
                    feature_control.words[0]);
  }
...
static void setMRs_vmexit(uint32_t reason, word_t qualification) {
  ...
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_CALL_EIP_MR,
        vmread(VMX_GUEST_RIP));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_CALL_CONTROL_PPC_MR,
        vmread(VMX_CONTROL_PRIMARY_PROCESSOR_CONTROLS));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_CALL_CONTROL_ENTRY_MR,
        vmread(VMX_CONTROL_ENTRY_INTERRUPTION_INFO));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_REASON_MR, reason);
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_QUALIFICATION_MR,
        qualification);

  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_INSTRUCTION_LEN_MR,
        vmread(VMX_DATA_EXIT_INSTRUCTION_LENGTH));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_GUEST_PHYSICAL_MR,
        vmread(VMX_DATA_GUEST_PHYSICAL));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_RFLAGS_MR,
        vmread(VMX_GUEST_RFLAGS));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_GUEST_INT_MR,
        vmread(VMX_GUEST_INTERRUPTABILITY));
  setMR(NODE_STATE(ksCurThread), buffer, SEL4_VMENTER_FAULT_CR3_MR,
...
static inline void finishVmexitSaving(void) {
  ...
  if (vcpuThreadUsingFPU(NODE_STATE(ksCurThread))) {
    /* If the vcpu owns the fpu then we did not modify the active cr0 to
     * anything different to what the VCPU owner requested, so we can update it
     * with any modifications the guest may have made */
    vcpu->cr0 = vcpu->cached_cr0;
  } else {
    /* If the vcpu does not own the fpu then we will have forced the task
     * switched flag to be set in the cr0 that was put into the vmcs. Since the
     * VCPU owner (or the guest) may believe the task switched flag is not set,
     * and the guest may have modified other parts of cr0 during execution then
     * to update the desired cr0 value to be the cr0 value from the vmcs (thus
     * pulling in any modifications the guest made) but removing the task
     * switched flag that we set and then adding back in the task switched flag
     * that may be in the desired current cr0 */
    vcpu->cr0 =
        (vcpu->cached_cr0 & ~CR0_TASK_SWITCH) |
...
exception_t handleVmexit(void) {
  ...
  if (!vcpuThreadUsingFPU(NODE_STATE(ksCurThread))) {
    /* since this vcpu does not currently own the fpu state, check if the kernel
     * should switch the fpu owner or not. We switch if the guest performed and
     * unimplemented device exception AND the owner of this vcpu has not
     * requested that these exceptions be forwarded to them (i.e. if they have
     * not explicitly set the unimplemented device exception in the
     * exception_bitmap) */
    if (reason == EXCEPTION_OR_NMI &&
        !(NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->exception_bitmap &
          BIT(int_unimpl_dev))) {
      interrupt = vmread(VMX_DATA_EXIT_INTERRUPT_INFO);
      /* The exception number is the bottom 8 bits of the interrupt info */
      if ((interrupt & 0xff) == int_unimpl_dev) {
        switchLocalFpuOwner(
            &NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->fpuState);
        return EXCEPTION_NONE;
      }
    } else if (reason == CONTROL_REGISTER &&
               !(NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0_mask &
                 CR0_TASK_SWITCH)) {
      /* we get here if the guest is attempting to write to a control register
       * that is set (by a 1 bit in the cr0 mask) as being owned by the host. If
       * we got here then the previous check on cr0_mask meant that the VCPU
       * owner did not claim ownership of the the task switch bit however we may
       * have temporarily claimed ownership for the purposes of FPU switching.
       * At this point we could still have a false positive, as the guest could
       * be attempted to manipulate bits that are not task switch, so we still
       * have to be careful and propogate all or some of an attempted write */
      ...
      /* We only care about some of the exit qualification cases, we handle them
       * here and will deliver any others through to fault handler */
      switch (
          vmx_data_exit_qualification_control_regster_get_access_type(qual)) {
        case VMX_EXIT_QUAL_TYPE_MOV_CR: {
          /* check for cr0 */
          if (vmx_data_exit_qualification_control_regster_get_cr(qual) == 0) {
            vcpu_gp_register_t source =
                crExitRegs[vmx_data_exit_qualification_control_regster_get_reg(
                    qual)];
            word_t value;
            if (source == VCPU_ESP) {
              /* ESP is the only register that is is not part of the general
               * purpose registers that we have to save and restore ourselves,
               * so we need to get this one from the vmcs */
              value = vmread(VMX_GUEST_RSP);
            } else {
              value = NODE_STATE(ksCurThread)
                          ->tcbArch.tcbVCPU->gp_registers[source];
            }
            /* First unset the task switch bit in cr0 */
            NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0 &= ~CR0_TASK_SWITCH;
            /* now set it to the value we were given */
            NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0 |=
                value & CR0_TASK_SWITCH;
            /* check if there are any parts of the write remaining to forward.
             * we only need to consider bits that the hardware will not have
             * handled without faulting, which is writing any bit such that it
             * is different to the shadow, but only considering bits that the
             * VCPU owner has declared that they want to own (via the
             * cr0_shadow)
             */
            if (!((value ^
                   NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0_shadow) &
                  NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0_mask)) {
              return EXCEPTION_NONE;
            }
          }
        ...
        case VMX_EXIT_QUAL_TYPE_LMSW: {
          uint16_t value =
              vmx_data_exit_qualification_control_regster_get_data(qual);
          /* First unset the task switch bit in cr0 */
          NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0 &= ~CR0_TASK_SWITCH;
          /* now set it to the value we were given */
          NODE_STATE(ksCurThread)->tcbArch.tcbVCPU->cr0 |=
              value & CR0_TASK_SWITCH;
          /* check if there are any parts of the write remaining to forward. we
           * only need to consider bits that the hardware will not have handled
           * without faulting, which is writing any bit such that it is
           * different to the shadow, but only considering bits that the VCPU
           * owner has declared that they want to own (via the cr0_shadow).
           * Additionally since LMSW only loads the bottom 4 bits of CR0 we only
           * consider the low 4 bits
           */
...
static void setEPTRoot(cap_t vmxSpace, vcpu_t *vcpu) {
  ...
  if (ept_root != vcpu->last_ept_root) {
    ...
    vmx_eptp_t eptp =
        vmx_eptp_new(ept_root, /* paddr of ept */
...
static void handleLazyFpu(void) {
   ...
   * and so we will put into the vmcs precisely what the VCPU owner has
   * requested */
  if (!vcpuThreadUsingFPU(NODE_STATE(ksCurThread))) {
    ...
    /* we need to claim ownership of the task switch bit so that any
     * modifications of it cause an exception and we must do this regardless of
     * whether the VCPU owner also wants to claim ownership. In the scenario
     * where the VCPU owner had also claimed ownership then in handleVmexit we
     * won't actually do any fpu switching and will forward the fault */
    cr0_mask |= CR0_TASK_SWITCH;
    /* since we have forced a value in the cr0 mask we need to set an
     * appropriate value in the cr0 read shadow. If the VCPU owner is also
     * masking this bit then we should use the value they have put in the cr0
     * read shadow. If they aren't then the read shadow will contain garbage and
     * we should instead set the read shadow to the actual desired cr0 value */
...
void restoreVMCS(void) {
  ...
  setEPTRoot(TCB_PTR_CTE_PTR(NODE_STATE(ksCurThread), tcbArchEPTRoot)->cap,
             expected_vmcs);
  handleLazyFpu();
}
...
void invept(ept_pml4e_t *ept_pml4) {
  if (vmx_ept_vpid_cap_msr_get_invept(vpid_capability)) {
    ...
    if (vmx_ept_vpid_cap_msr_get_invept_single_context(vpid_capability)) {
      ...
      userError(
          "Hardware claimed to support invept, yet provided no mechanism");
      return;
    }

    address.parts[0] = pptr_to_paddr((void *)ept_pml4);
    address.parts[1] = 0;
    asm volatile("invept %0, %1" : : "m"(address), "r"(type) : "memory");
  }
}

#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/arch/x86/smp/ipi.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline void init_ipi_args(IpiRemoteCall_t func, word_t data1,
                                 word_t data2, word_t data3, word_t mask) {
...
compile_assert(invalid_number_of_supported_nodes,
               CONFIG_MAX_NUM_NODES <= wordBits);
...
static void x86_ipi_send_mask(interrupt_t ipi, word_t mask, bool_t isBlocking) {
  ...
  do {
    ...
    target_clusters[nr_target_clusters] |=
        cpu_mapping.index_to_logical_id[core];
    if (isBlocking) {
    ...
    while (sub_mask) {
      ...
      target_clusters[nr_target_clusters] |=
          cpu_mapping.index_to_logical_id[index];
      if (isBlocking) {
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/assert.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void _assert_fail(const char *assertion, const char *file, unsigned int line,
                  const char *function) {
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/benchmark/benchmark.c"
/*
 * Copyright 2016, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
exception_t handle_SysBenchmarkDumpAllThreadsUtilisation(void) {
  ...
  printf("  \"BENCHMARK_TOTAL_UTILISATION\":%lu,\n",
         (word_t)(NODE_STATE(benchmark_end_time) -
                  NODE_STATE(benchmark_start_time)));
  ...
  for (tcb_t *curr = NODE_STATE(ksDebugTCBs); curr != NULL;
       curr = TCB_PTR_DEBUG_PTR(curr)->tcbDebugNext) {
    printf("    {\n");
...
#endif /* CONFIG_BENCHMARK_TRACK_UTILISATION */
#endif /* CONFIG_ENABLE_BENCHMARKS */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/benchmark/benchmark_track.c"
/*
 * Copyright 2016, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void benchmark_track_exit(void) {
  ...
  benchmark_track_kernel_entry_t *ksLog =
      (benchmark_track_kernel_entry_t *)KS_LOG_PPTR;
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/benchmark/benchmark_utilisation.c"
/*
 * Copyright 2016, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void benchmark_track_utilisation_dump(void) {
  uint64_t *buffer = ((uint64_t *)&(
      ((seL4_IPCBuffer *)lookupIPCBuffer(true, NODE_STATE(ksCurThread)))
          ->msg[0]));
  tcb_t *tcb = NULL;
  word_t tcb_cptr = getRegister(NODE_STATE(ksCurThread), capRegister);
  lookupCap_ret_t lu_ret;
  ...
  buffer[BENCHMARK_TCB_UTILISATION] =
      tcb->benchmark.utilisation; /* Requested thread utilisation */
  buffer[BENCHMARK_TCB_NUMBER_SCHEDULES] =
      tcb->benchmark.number_schedules; /* Number of times scheduled */
  buffer[BENCHMARK_TCB_KERNEL_UTILISATION] =
      tcb->benchmark.kernel_utilisation; /* Utilisation spent in kernel */
  buffer[BENCHMARK_TCB_NUMBER_KERNEL_ENTRIES] =
      tcb->benchmark.number_kernel_entries; /* Number of kernel entries */

  /* Idle counters */
  buffer[BENCHMARK_IDLE_LOCALCPU_UTILISATION] =
      NODE_STATE(ksIdleThread)
          ->benchmark.utilisation; /* Idle thread utilisation of current CPU */
#ifdef ENABLE_SMP_SUPPORT
  buffer[BENCHMARK_IDLE_TCBCPU_UTILISATION] =
      NODE_STATE_ON_CORE(ksIdleThread, tcb->tcbAffinity)
          ->benchmark.utilisation; /* Idle thread utilisation of CPU the TCB is
                                      running on */
#else
  buffer[BENCHMARK_IDLE_TCBCPU_UTILISATION] =
      buffer[BENCHMARK_IDLE_LOCALCPU_UTILISATION];
  ...
  buffer[BENCHMARK_IDLE_NUMBER_SCHEDULES] =
      NODE_STATE(ksIdleThread)
          ->benchmark.number_schedules; /* Number of times scheduled */
  buffer[BENCHMARK_IDLE_KERNEL_UTILISATION] =
      NODE_STATE(ksIdleThread)
          ->benchmark.kernel_utilisation; /* Utilisation spent in kernel */
  buffer[BENCHMARK_IDLE_NUMBER_KERNEL_ENTRIES] =
      NODE_STATE(ksIdleThread)
          ->benchmark.number_kernel_entries; /* Number of kernel entries */

  /* Total counters */
#ifdef CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT
  buffer[BENCHMARK_TOTAL_UTILISATION] =
      (NODE_STATE(ccnt_num_overflows) * 0xFFFFFFFFU) +
      NODE_STATE(benchmark_end_time) - NODE_STATE(benchmark_start_time);
#else
  buffer[BENCHMARK_TOTAL_UTILISATION] =
      NODE_STATE(benchmark_end_time) -
      NODE_STATE(benchmark_start_time); /* Overall time */
#endif /* CONFIG_ARM_ENABLE_PMU_OVERFLOW_INTERRUPT */
  buffer[BENCHMARK_TOTAL_NUMBER_SCHEDULES] =
      NODE_STATE(benchmark_kernel_number_schedules);
  buffer[BENCHMARK_TOTAL_KERNEL_UTILISATION] =
      NODE_STATE(benchmark_kernel_time);
...
#endif /* CONFIG_BENCHMARK_TRACK_UTILISATION */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/fastpath/fastpath.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline FORCE_INLINE
  ...
  /* Lookup the cap */
  ep_cap =
      lookup_fp(TCB_PTR_CTE_PTR(NODE_STATE(ksCurThread), tcbCTable)->cap, cptr);
  ...
  /* stored_hw_asid is unused on ia32 fastpath, but gets passed into a function
   * below. */
  ...
  if (unlikely(asid_map_get_type(asid_map) != asid_map_asid_map_vspace ||
               VSPACE_PTR(asid_map_asid_map_vspace_get_vspace_root(asid_map)) !=
                   cap_pd)) {
    slowpath(SysCall);
  }
  ...
  stored_hw_asid.words[0] =
      asid_map_asid_map_vspace_get_stored_hw_vmid(asid_map);
#else
  ...
  /* ensure only the idle thread or lower prio threads are present in the
   * scheduler */
  ...
  /* Ensure the original caller is in the current domain and can be scheduled
   * directly. */
  ...
  thread_state_ptr_set_replyObject_np(&NODE_STATE(ksCurThread)->tcbState,
                                      REPLY_REF(reply));
  ...
  word_t replyCanGrant =
      thread_state_ptr_get_blockingIPCCanGrant(&dest->tcbState);
  ;
...
#endif
{
  ...
  cap_t reply_cap = lookup_fp(
      TCB_PTR_CTE_PTR(NODE_STATE(ksCurThread), tcbCTable)->cap, reply);
  ...
  if (unlikely(reply_ptr->replyTCB == NULL ||
               ...
               SC_PTR(call_stack_get_callStackPtr(reply_ptr->replyNext)) !=
                   NODE_STATE(ksCurThread)->tcbSchedContext)) {
    slowpath(SysReplyRecv);
  }
...
#if defined(CONFIG_HARDWARE_DEBUG_API) && defined(CONFIG_ARCH_IA32)
  if (unlikely(
          caller->tcbArch.tcbContext.breakpointState.single_step_enabled)) {
    slowpath(SysReplyRecv);
  }
#endif
  ...
  thread_state_ptr_mset_blockingObject_tsType(
      &NODE_STATE(ksCurThread)->tcbState, (word_t)ep_ptr,
  ...
  thread_state_ptr_set_blockingIPCCanGrant(
      &NODE_STATE(ksCurThread)->tcbState,
  ...
  if (likely(!endpointTail)) {
    ...
    endpoint_ptr_mset_epQueue_tail_state(
        ep_ptr, TCB_REF(NODE_STATE(ksCurThread)), EPState_Recv);
  } else {
#ifdef CONFIG_KERNEL_MCS
    /* Update queue. */
    tcb_queue_t queue =
        tcbEPAppend(NODE_STATE(ksCurThread), ep_ptr_get_queue(ep_ptr));
    endpoint_ptr_set_epQueue_head_np(ep_ptr, TCB_REF(queue.head));
    endpoint_ptr_mset_epQueue_tail_state(ep_ptr, TCB_REF(queue.end),
                                         EPState_Recv);
#else
  ...
  mdb_node_ptr_mset_mdbNext_mdbRevocable_mdbFirstBadged(
      &CTE_PTR(mdb_node_get_mdbPrev(callerSlot->cteMDBNode))->cteMDBNode, 0, 1,
      1);
  ...
  if (unlikely(fault_type != seL4_Fault_NullFault)) {
    /* Note - this works as is for VM faults but will need to be changed when
     * other faults are added. VM faults always restart the faulting thread upon
     * reply but this is not always the case with other types of faults. This
     * can either be handled in the fastpath or redirected to the slowpath, but
     * either way, this code must be changed so we do not forcefully switch to a
     * thread which is meant to stay inactive. */

    /* In the slowpath, the thread is set to ThreadState_Restart and its PC is
     * set to its restartPC in activateThread(). In the fastpath, this step is
     * bypassed and we directly complete the activateThread() steps that set the
     * PC and make the thread runnable. */
    ...
    /* The badge/msginfo do not need to be not sent - this is not necessary for
     * exceptions */
    restore_user_context();
  } else {
#endif
...
static inline FORCE_INLINE
#endif
    void NORETURN
    fastpath_signal(word_t cptr, word_t msgInfo) {
  word_t fault_type;
  ...
  switch (ntfnState) {
    ...
    case NtfnState_Idle:
      ...
      if (!dest || thread_state_ptr_get_tsType(&dest->tcbState) !=
                       ThreadState_BlockedOnReceive) {
#ifdef CONFIG_BENCHMARK_TRACK_KERNEL_ENTRIES
        ksKernelEntry.is_fastpath = true;
#endif
  ...
  /* Only fastpath signal to threads which will not become the new highest prio
   * thread on the core of their SC, even if the currently running thread on the
   * core is the idle thread. */
  if (NODE_STATE_ON_CORE(ksCurThread, sc->scCore)->tcbPriority <
      dest->tcbPriority) {
    slowpath(SysSend);
  }

  /* Simplified schedContext_resume that does not change state and reverts to
   * the slowpath in cases where the SC does not have sufficient budget, as this
   * case adds extra scheduler logic. Normally, this is done after donation of
   * SC but after tweaking it, I don't see anything executed in
   * schedContext_donate that will affect the conditions of this check */
  if (sc->scRefillMax > 0) {
  ...
  if (ksCurDomain != dest->tcbDomain SMP_COND_STATEMENT(
...
#ifdef CONFIG_EXCEPTION_FASTPATH
static inline FORCE_INLINE void NORETURN
fastpath_vm_fault(vm_fault_type_t type) {
  cap_t handler_cap;
  endpoint_t *ep_ptr;
  tcb_t *dest;
  cap_t newVTable;
  ...
  handler_cap = lookup_fp(
      TCB_PTR_CTE_PTR(NODE_STATE(ksCurThread), tcbCTable)->cap, handlerCPtr);
#endif

  /* Check that the cap is an endpoint cap and on non-mcs, that you can send to
   * it and create the reply cap */
  if (unlikely(!cap_capType_equals(handler_cap, cap_endpoint_cap)
#ifndef CONFIG_KERNEL_MCS
               || !cap_endpoint_cap_get_capCanSend(handler_cap) ||
               (!cap_endpoint_cap_get_capCanGrant(handler_cap) &&
  ...
  /* Sets the tcb fault based on the vm fault information. Has one slowpath
  transition but only for a debug fault on AARCH32 */
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/inlines.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/kernel/boot.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#endif
  /* for all archs, seL4_PageTable Bits is the size of all non top-level paging
   * structures */
  return size + arch_get_n_paging(it_v_reg) * BIT(seL4_PageTableBits);
}

BOOT_CODE static void maybe_alloc_extra_bi(word_t cmp_size_bits,
                                           word_t extra_bi_size_bits) {
 ...
 */
BOOT_CODE static void create_rootserver_objects(pptr_t start,
                                                v_region_t it_v_reg,
                                                word_t extra_bi_size_bits) {
  ...
  compile_assert(invalid_BI_FRAME_SIZE_BITS,
                 BI_FRAME_SIZE_BITS == seL4_PageBits);
...
compile_assert(root_cnode_size_valid,
               CONFIG_ROOT_CNODE_SIZE_BITS < 32 - seL4_SlotBits &&
                   ...
                   BIT(CONFIG_ROOT_CNODE_SIZE_BITS) >=
                       (seL4_PageBits - seL4_SlotBits))

    BOOT_CODE cap_t create_root_cnode(void) {
  cap_t cap =
      cap_cnode_cap_new(CONFIG_ROOT_CNODE_SIZE_BITS,            /* radix */
...
BOOT_CODE cap_t create_ipcbuf_frame_cap(cap_t root_cnode_cap, cap_t pd_cap,
                                        vptr_t vptr) {
  ...
  cap_t cap = create_mapped_it_frame_cap(pd_cap, rootserver.ipc_buf, vptr,
                                         IT_ASID, false, false);
  write_slot(SLOT_PTR(pptr_of_cap(root_cnode_cap), seL4_CapInitThreadIPCBuffer),
             cap);

  return cap;
}

BOOT_CODE void create_bi_frame_cap(cap_t root_cnode_cap, cap_t pd_cap,
                                   vptr_t vptr) {
  /* create a cap of it and write it into the root CNode */
...
create_frames_of_region(cap_t root_cnode_cap, cap_t pd_cap, region_t reg,
                        bool_t do_map, sword_t pv_offset) {
  pptr_t f;
  cap_t frame_cap;
  seL4_SlotPos slot_pos_before;
  seL4_SlotPos slot_pos_after;
  ...
  for (f = reg.start; f < reg.end; f += BIT(PAGE_BITS)) {
    if (do_map) {
      frame_cap = create_mapped_it_frame_cap(
          pd_cap, f, pptr_to_paddr((void *)(f - pv_offset)), IT_ASID, false,
          true);
    } else {
      frame_cap = create_unmapped_it_frame_cap(f, false);
    }
    if (!provide_cap(root_cnode_cap, frame_cap)) {
      return (create_frames_of_region_ret_t){.region = S_REG_EMPTY,
                                             .success = false};
    }
  }
  ...
  return (create_frames_of_region_ret_t){
      .region =
          (seL4_SlotRegion){.start = slot_pos_before, .end = slot_pos_after},
      .success = true};
}

BOOT_CODE cap_t create_it_asid_pool(cap_t root_cnode_cap) {
  cap_t ap_cap =
      cap_asid_pool_cap_new(IT_ASID >> asidLowBits, rootserver.asid_pool);
...
#ifdef CONFIG_KERNEL_MCS
BOOT_CODE static void configure_sched_context(tcb_t *tcb,
                                              sched_context_t *sc_pptr,
                                              ticks_t timeslice) {
...
BOOT_CODE bool_t init_sched_control(cap_t root_cnode_cap, word_t num_nodes) {
  ...
  ndks_boot.bi_frame->schedcontrol = (seL4_SlotRegion){
      .start = slot_pos_before, .end = ndks_boot.slot_pos_cur};
...
BOOT_CODE tcb_t *create_initial_thread(cap_t root_cnode_cap, cap_t it_pd_cap,
                                       vptr_t ui_v_entry, vptr_t bi_frame_vptr,
  ...
  deriveCap_ret_t dc_ret = deriveCap(
  ...
  /* initialise TCB */
#ifdef CONFIG_KERNEL_MCS
  configure_sched_context(tcb, SC_PTR(rootserver.sc),
                          usToTicks(CONFIG_BOOT_THREAD_TIME_SLICE * US_IN_MS));
#endif
 ...
 */
BOOT_CODE static bool_t provide_untyped_cap(cap_t root_cnode_cap,
                                            bool_t device_memory, pptr_t pptr,
                                            word_t size_bits,
  ...
  if (!device_memory && !pptr_in_kernel_window(pptr + MASK(size_bits))) {
    printf(
        "Kernel init: End of non-device untyped at %p outside kernel window "
        "(size %" SEL4_PRIu_word ")\n",
        (void *)pptr, size_bits);
    return false;
  }
  ...
  if (i < CONFIG_MAX_NUM_BOOTINFO_UNTYPED_CAPS) {
    ndks_boot.bi_frame->untypedList[i] =
        (seL4_UntypedDesc){.paddr = pptr_to_paddr((void *)pptr),
                           .sizeBits = size_bits,
                           .isDevice = device_memory,
                           .padding = {0}};
    ut_cap = cap_untyped_cap_new(MAX_FREE_INDEX(size_bits), device_memory,
                                 size_bits, pptr);
 ...
 *
 * Takes care of alignement, size and potentially wrapping memory regions. It is
 * fine to provide a region with end < start if the memory is device memory.
 *
 * If the region start is not aligned to seL4_MinUntypedBits, the part up to the
 * next aligned address will be ignored and is lost, because it is too small to
 * create kernel objects in.
 *
...
BOOT_CODE static bool_t create_untypeds_for_region(
    cap_t root_cnode_cap, bool_t device_memory, region_t reg,
    seL4_SlotPos first_untyped_slot) {
  /* This code works with regions that wrap (where end < start), because the
     ...
     alignement, size, and overflow conditionds. The region [0..end) is not
     neccessarily part of the kernel window (depending on the value of
  ...
  while (!is_reg_empty(reg)) {
    /* Calculate the bit size of the region. This is also correct for end <
       start: it will return the correct size of the set [start..-1] union
       [0..end). This will then be too large for alignment, so the code further
...
BOOT_CODE bool_t create_untypeds(cap_t root_cnode_cap,
  ...
  for (word_t i = 0; i < ndks_boot.resv_count; i++) {
    if (start < ndks_boot.reserved[i].start) {
      ...
      if (!create_untypeds_for_region(root_cnode_cap, true, reg,
                                      first_untyped_slot)) {
        printf(
            "ERROR: creation of untypeds for device region #%u at"
  ...
  if (start < CONFIG_PADDR_USER_DEVICE_TOP) {
    region_t reg =
        paddr_to_pptr_reg((p_region_t){start, CONFIG_PADDR_USER_DEVICE_TOP});
  ...
  /* if boot_mem_reuse_reg is not empty, we can create UT objs from boot
   * code/data frames */
  ...
  for (word_t i = 0; i < ARRAY_SIZE(ndks_boot.freemem); i++) {
    ...
    if (!create_untypeds_for_region(root_cnode_cap, false, reg,
                                    first_untyped_slot)) {
      printf(
          "ERROR: creation of untypeds for free memory region #%u at"
          " [%" SEL4_PRIx_word "..%" SEL4_PRIx_word "] failed\n",
          (unsigned int)i, reg.start, reg.end);
      return false;
    }
  ...
  ndks_boot.bi_frame->untyped = (seL4_SlotRegion){
      .start = first_untyped_slot, .end = ndks_boot.slot_pos_cur};
...
BOOT_CODE void bi_finalise(void) {
  ndks_boot.bi_frame->empty = (seL4_SlotRegion){
      .start = ndks_boot.slot_pos_cur, .end = BIT(CONFIG_ROOT_CNODE_SIZE_BITS)};
}
...
BOOT_CODE bool_t init_freemem(word_t n_available, const p_region_t *available,
  ...
  while (a < n_available && r < n_reserved) {
    if (reserved[r].start == reserved[r].end) {
      ...
      /* the reserved region is above the available region - take the whole
       * thing */
  ...
  printf(
      "ERROR: no free memory region is big enough for root server "
      "objects, need size/alignment of 2^%" SEL4_PRIu_word "\n",
      max);
  return false;
}
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/kernel/cspace.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
//      */
//     guard =
//         (capptr >> ((n_bits - guardBits) & MASK(wordRadix))) & MASK(guardBits);
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/kernel/faulthandler.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void handleFault(tcb_t *tptr) {
  bool_t hasFaultHandler =
      sendFaultIPC(tptr, TCB_PTR_CTE_PTR(tptr, tcbFaultHandler)->cap,
...
bool_t sendFaultIPC(tcb_t *tptr, cap_t handlerCap, bool_t can_donate) {
  if (cap_get_capType(handlerCap) == cap_endpoint_cap) {
    ...
    sendIPC(true, false, cap_endpoint_cap_get_capEPBadge(handlerCap),
            cap_endpoint_cap_get_capCanGrant(handlerCap),
            cap_endpoint_cap_get_capCanGrantReply(handlerCap), can_donate, tptr,
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/kernel/stack.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/kernel/thread.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void activateThread(void) {
  ...
  if (unlikely(NODE_STATE(ksCurThread)->tcbYieldTo)) {
    ...
    assert(thread_state_get_tsType(NODE_STATE(ksCurThread)->tcbState) ==
           ThreadState_Running);
  }
#endif
...
void suspend(tcb_t *target) {
  ...
  if (thread_state_get_tsType(target->tcbState) == ThreadState_Running) {
     ...
     * after we make it inactive we update its restart pc so that the thread
     * next runs at the correct address whether it is restarted or moved
     * directly to running */
...
void doIPCTransfer(tcb_t *sender, endpoint_t *endpoint, word_t badge,
  ...
  if (likely(seL4_Fault_get_seL4_FaultType(sender->tcbFault) ==
...
#endif
{
  ...
  if (reply->replyTCB == NULL ||
      thread_state_get_tsType(reply->replyTCB->tcbState) !=
          ThreadState_BlockedOnReply) {
    /* nothing to do */
    return;
  }
  ...
  if (sc_sporadic(receiver->tcbSchedContext) &&
      receiver->tcbSchedContext !=
          NODE_STATE_ON_CORE(ksCurSC, receiver->tcbSchedContext->scCore)) {
  ...
  if (receiver->tcbSchedContext && isRunnable(receiver)) {
    if ((refill_ready(receiver->tcbSchedContext) &&
      ...
      if (validTimeoutHandler(receiver) && fault_type != seL4_Fault_Timeout) {
        current_fault =
            seL4_Fault_Timeout_new(receiver->tcbSchedContext->scBadge);
...
static seL4_MessageInfo_t transferCaps(seL4_MessageInfo_t info,
  ...
  for (i = 0;
       i < seL4_MsgMaxExtraCaps && current_extra_caps.excaprefs[i] != NULL;
       i++) {
...
static void switchSchedContext(void) {
  if (unlikely(NODE_STATE(ksCurSC) !=
               NODE_STATE(ksCurThread)->tcbSchedContext)) {
    NODE_STATE(ksReprogram) = true;
  ...
  if (NODE_STATE(ksSchedulerAction) != SchedulerAction_ResumeCurrentThread) {
    ...
    if (NODE_STATE(ksSchedulerAction) == SchedulerAction_ChooseNewThread) {
      ...
      if (fastfail && !isHighestPrio(ksCurDomain, candidate->tcbPriority)) {
        ...
        /* We append the candidate at the end of the scheduling queue, that way
         * the current thread, that was enqueued at the start of the scheduling
         * queue will get picked during chooseNewThread */
...
#ifdef CONFIG_BENCHMARK_TRACK_UTILISATION
  benchmark_utilisation_switch(NODE_STATE(ksCurThread),
                               NODE_STATE(ksIdleThread));
#endif
...
void setPriority(tcb_t *tptr, prio_t prio) {
  switch (thread_state_get_tsType(tptr->tcbState)) {
    ...
    case ThreadState_BlockedOnNotification:
      tptr->tcbPriority = prio;
      reorderNTFN(NTFN_PTR(thread_state_get_blockingObject(tptr->tcbState)),
                  tptr);
      break;
    default:
      tptr->tcbPriority = prio;
...
#endif
    if (ksCurDomain != target->tcbDomain SMP_COND_STATEMENT(
                           || target->tcbAffinity != getCurrentCPUIndex())) {
      SCHED_ENQUEUE(target);
    } else if (NODE_STATE(ksSchedulerAction) !=
...
void setNextInterrupt(void) {
  ...
  if (NODE_STATE(ksReleaseHead) != NULL) {
    next_interrupt =
        MIN(refill_head(NODE_STATE(ksReleaseHead)->tcbSchedContext)->rTime,
            next_interrupt);
  }
...
void chargeBudget(ticks_t consumed, bool_t canTimeoutFault) {
  if (likely(NODE_STATE(ksCurSC) != NODE_STATE(ksIdleSC))) {
    if (isRoundRobin(NODE_STATE(ksCurSC))) {
      ...
      refill_head(NODE_STATE(ksCurSC))->rAmount +=
          refill_tail(NODE_STATE(ksCurSC))->rAmount;
...
#ifdef CONFIG_KERNEL_MCS
void awaken(void) {
  while (unlikely(NODE_STATE(ksReleaseHead) != NULL &&
                  refill_ready(NODE_STATE(ksReleaseHead)->tcbSchedContext))) {
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/machine/capdl.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
/* TBD: This is to avoid traversing the same cnode. It should be applied to
 * object as well since the extractor might comes across multiple caps to the
 * same object.
 */
...
void obj_tcb_print_attrs(tcb_t *tcb) {
  printf("(addr: 0x%lx, ip: 0x%lx, sp: 0x%lx, prio: %lu, max_prio: %lu",
         ...
         (long unsigned int)tcb->tcbPriority, (long unsigned int)tcb->tcbMCP);
...
void obj_tcb_print_slots(tcb_t *tcb) {
  ...
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbCTable)->cap) != cap_null_cap) {
    printf("cspace: %p_cnode ", (void *)cap_cnode_cap_get_capCNodePtr(
                                    TCB_PTR_CTE_PTR(tcb, tcbCTable)->cap));
    cap_cnode_print_attrs(TCB_PTR_CTE_PTR(tcb, tcbCTable)->cap);
  }

  /* VSpace root */
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbVTable)->cap) != cap_null_cap) {
    printf("vspace: %p_pd\n", cap_vtable_cap_get_vspace_root_fp(
                                  TCB_PTR_CTE_PTR(tcb, tcbVTable)->cap));
  }
  ...
  /* Fault endpoint slot */
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbFaultHandler)->cap) !=
      cap_null_cap) {
    printf("fault_ep_slot: %p_ep ",
           (void *)cap_endpoint_cap_get_capEPPtr(
               TCB_PTR_CTE_PTR(tcb, tcbFaultHandler)->cap));
  ...
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbTimeoutHandler)->cap) !=
      cap_null_cap) {
    printf("temp_fault_ep_slot: %p_ep ",
           (void *)cap_endpoint_cap_get_capEPPtr(
               TCB_PTR_CTE_PTR(tcb, tcbTimeoutHandler)->cap));
  ...
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbReply)->cap) != cap_null_cap) {
    printf("reply_slot: %p_reply\n", (void *)cap_reply_cap_get_capTCBPtr(
                                         TCB_PTR_CTE_PTR(tcb, tcbReply)->cap));
  }

  /* TCB of most recent IPC sender */
  if (cap_get_capType(TCB_PTR_CTE_PTR(tcb, tcbCaller)->cap) != cap_null_cap) {
    tcb_t *caller = TCB_PTR(
        cap_thread_cap_get_capTCBPtr(TCB_PTR_CTE_PTR(tcb, tcbCaller)->cap));
...
void print_objects(void) {
  ...
  for (tcb_t *curr = NODE_STATE(ksDebugTCBs); curr != NULL;
    ...
    if (cap_get_capType(TCB_PTR_CTE_PTR(curr, tcbCTable)->cap) ==
        cap_cnode_cap) {
  ...
  switch (cap_get_capType(cap)) {
    ...
    case cap_irq_handler_cap: {
      printf("%p_%lu_irq\n", (void *)cap_irq_handler_cap_get_capIRQ(cap),
             (long unsigned int)IRQT_TO_CORE(
...
void print_object(cap_t cap) {
  switch (cap_get_capType(cap)) {
    ...
    case cap_sched_context_cap: {
      printf("%p_sc = sc ", (void *)cap_sched_context_cap_get_capSCPtr(cap));
...
#endif /* CONFIG_DEBUG_BUILD */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/machine/fpu.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/machine/io.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
 ...
 * passes NULL for 'buf'. */
static void do_output_to_putchar(UNUSED out_wrap_t *out, const char *buf,
                                 word_t len) {
  if (buf) {
    while (len-- > 0) {
      putchar(*buf++);
...
static inline bool_t isdigit(char c) { return c >= '0' && c <= '9'; }
...
static char *fmt_u(uintmax_t x, char *s) {
  while (0 != x) {
#if defined(CONFIG_ARCH_AARCH32) || defined(CONFIG_ARCH_RISCV32) || \
...
static int printf_core(out_wrap_t *f, const char *fmt, va_list *ap,
...
int impl_kvprintf(const char *format, va_list ap) {
  out_wrap_t out_wrap = {
      .write = do_output_to_putchar, .buf = NULL, .maxlen = 0, .used = 0};
...
int impl_ksnvprintf(char *str, word_t size, const char *format, va_list ap) {
  ...
  out_wrap_t out_wrap = {
      .write = do_output_to_buffer, .buf = str, .maxlen = size, .used = 0};
...
#endif /* CONFIG_PRINTING */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/machine/registerset.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/model/preemption.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#ifdef CONFIG_KERNEL_MCS
    updateTimestamp();
    if (!(sc_active(NODE_STATE(ksCurSC)) &&
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/model/smp.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/model/statedata.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
/* Collective cpu states, including both pre core architecture dependant and
 * independent data */
SMP_STATE_DEFINE(smpStatedata_t,
                 ksSMP[CONFIG_MAX_NUM_NODES] ALIGN(L1_CACHE_LINE_SIZE));
...
UP_STATE_DEFINE(word_t,
                ksReadyQueuesL2Bitmap[CONFIG_NUM_DOMAINS][L2_BITMAP_SIZE]);
...
cte_t intStateIRQNode[BIT(IRQ_CNODE_SLOT_BITS)] ALIGN(BIT(IRQ_CNODE_SLOT_BITS +
                                                          seL4_SlotBits));
compile_assert(irqCNodeSize, sizeof(intStateIRQNode) >=
                                 ((INT_STATE_ARRAY_SIZE) * sizeof(cte_t)));
...
char ksIdleThreadTCB[CONFIG_MAX_NUM_NODES][BIT(seL4_TCBBits)] ALIGN(
    BIT(TCB_SIZE_BITS));

#ifdef CONFIG_KERNEL_MCS
/* Idle thread Schedcontexts */
char ksIdleThreadSC[CONFIG_MAX_NUM_NODES][BIT(seL4_MinSchedContextBits)] ALIGN(
...
#endif /* CONFIG_KERNEL_LOG_BUFFER */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/cnode.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
exception_t decodeCNodeInvocation(word_t invLabel, word_t length, cap_t cap,
  ...
  if (invLabel == CNodeRotate) {
    ...
    setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
    return invokeCNodeRotate(newSrcCap, newPivotCap, srcSlot, pivotSlot,
                             destSlot);
  }
...
extern exception_t invokeCNodeRevoke(cte_t *destSlot); 
// { return cteRevoke(destSlot); }

extern exception_t invokeCNodeDelete(cte_t *destSlot);
//  {
//   return cteDelete(destSlot, true);
// }

extern exception_t invokeCNodeCancelBadgedSends(cap_t cap);
//  {
...
extern exception_t invokeCNodeInsert(cap_t cap, cte_t *srcSlot, cte_t *destSlot);
//  {
...
extern exception_t invokeCNodeMove(cap_t cap, cte_t *srcSlot, cte_t *destSlot);
//  {
...
extern exception_t invokeCNodeRotate(cap_t cap1, cap_t cap2, cte_t *slot1,
...
#ifndef CONFIG_KERNEL_MCS
extern exception_t invokeCNodeSaveCaller(cte_t *destSlot);
// {
//   cap_t cap;
//   cte_t *srcSlot;
 ...
 * parent.
 */
extern void setUntypedCapAsFull(cap_t srcCap, cap_t newCap, cte_t *srcSlot);
//  {
...
extern void cteInsert(cap_t newCap, cte_t *srcSlot, cte_t *destSlot);
//  {
...
//     mdb_node_ptr_set_mdbPrev(&CTE_PTR(mdb_node_get_mdbNext(newMDB))->cteMDBNode,
//                              CTE_REF(destSlot));
//   }
// }

extern void cteMove(cap_t newCap, cte_t *srcSlot, cte_t *destSlot);
//  {
//   mdb_node_t mdb;
...
//   if (prev_ptr)
//     mdb_node_ptr_set_mdbNext(&CTE_PTR(prev_ptr)->cteMDBNode, CTE_REF(destSlot));

//   next_ptr = mdb_node_get_mdbNext(mdb);
//   if (next_ptr)
//     mdb_node_ptr_set_mdbPrev(&CTE_PTR(next_ptr)->cteMDBNode, CTE_REF(destSlot));
// }

extern void capSwapForDelete(cte_t *slot1, cte_t *slot2);
//  {
//   cap_t cap1, cap2;
...
extern void cteSwap(cap_t cap1, cte_t *slot1, cap_t cap2, cte_t *slot2);
//  {
...
//   if (prev_ptr)
//     mdb_node_ptr_set_mdbNext(&CTE_PTR(prev_ptr)->cteMDBNode, CTE_REF(slot2));

//   next_ptr = mdb_node_get_mdbNext(mdb1);
//   if (next_ptr)
//     mdb_node_ptr_set_mdbPrev(&CTE_PTR(next_ptr)->cteMDBNode, CTE_REF(slot2));
...
//   if (prev_ptr)
//     mdb_node_ptr_set_mdbNext(&CTE_PTR(prev_ptr)->cteMDBNode, CTE_REF(slot1));

//   next_ptr = mdb_node_get_mdbNext(mdb2);
//   if (next_ptr)
//     mdb_node_ptr_set_mdbPrev(&CTE_PTR(next_ptr)->cteMDBNode, CTE_REF(slot1));
// }

extern exception_t cteRevoke(cte_t *slot);
//  {
//   cte_t *nextPtr;
//   exception_t status;
...
extern exception_t cteDelete(cte_t *slot, bool_t exposed);
//  {
...
extern void emptySlot(cte_t *slot, cap_t cleanupInfo);
//  {
...
extern bool_t capRemovable(cap_t cap, cte_t *slot);
...
extern bool_t capCyclicZombie(cap_t cap, cte_t *slot);
...
extern finaliseSlot_ret_t finaliseSlot(cte_t *slot, bool_t immediate);
...
extern exception_t reduceZombie(cte_t *slot, bool_t immediate);
...
extern void cteDeleteOne(cte_t *slot);
...
//         \<or> gs_get_assn cteDeleteOne_'proc \<acute>ghost'state =
//        \<acute>cap_type, id)" */
...
extern void insertNewCap(cte_t *parent, cte_t *slot, cap_t cap);
//  {
//   cte_t *next;
...
#ifndef CONFIG_KERNEL_MCS
extern void setupReplyMaster(tcb_t *thread);
//  {
//   cte_t *slot;
...
extern bool_t isMDBParentOf(cte_t *cte_a, cte_t *cte_b);
...
extern exception_t ensureNoChildren(cte_t *slot);
//  {
...
extern exception_t ensureEmptySlot(cte_t *slot);
//  {
//   if (cap_get_capType(slot->cap) != cap_null_cap) {
//     current_syscall_error.type = seL4_DeleteFirst;
//     return EXCEPTION_SYSCALL_ERROR;
...
extern bool_t PURE isFinalCapability(cte_t *cte);
...
extern bool_t slotCapLongRunningDelete(cte_t *slot);
...
extern cte_t *getReceiveSlots(tcb_t *thread, word_t *buffer);
//  {
//   cap_transfer_t ct;
//   cptr_t cptr;
...
extern cap_transfer_t loadCapTransfer(word_t *buffer);
// cap_transfer_t PURE loadCapTransfer(word_t *buffer) {
//   const int offset = seL4_MsgMaxLength + seL4_MsgMaxExtraCaps + 2;
//   return capTransferFromWords(buffer + offset);
// }
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/endpoint.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#ifdef CONFIG_KERNEL_MCS
void sendIPC(bool_t blocking, bool_t do_call, word_t badge, bool_t canGrant,
             bool_t canGrantReply, bool_t canDonate, tcb_t *thread,
             endpoint_t *epptr)
#else
void sendIPC(bool_t blocking, bool_t do_call, word_t badge, bool_t canGrant,
             bool_t canGrantReply, tcb_t *thread, endpoint_t *epptr)
#endif
{
  switch (endpoint_ptr_get_state(epptr)) {
    ...
    case EPState_Recv: {
      ...
      if (do_call || seL4_Fault_ptr_get_seL4_FaultType(&thread->tcbFault) !=
                         seL4_Fault_NullFault) {
      ...
      assert(dest->tcbSchedContext == NULL ||
             refill_sufficient(dest->tcbSchedContext, 0));
      assert(dest->tcbSchedContext == NULL ||
             refill_ready(dest->tcbSchedContext));
    ...
    switch (endpoint_ptr_get_state(epptr)) {
      ...
      case EPState_Recv: {
        ...
        if (isBlocking) {
          ...
          thread_state_ptr_set_blockingObject(&thread->tcbState, EP_REF(epptr));
#ifdef CONFIG_KERNEL_MCS
          thread_state_ptr_set_replyObject(&thread->tcbState,
                                           REPLY_REF(replyPtr));
          if (replyPtr) {
      ...
      case EPState_Send: {
        ...
        if (do_call || seL4_Fault_get_seL4_FaultType(sender->tcbFault) !=
          ...
          assert(sender->tcbSchedContext == NULL ||
                 refill_sufficient(sender->tcbSchedContext, 0));
        }
#else
        if (do_call) {
          if (canGrant || canGrantReply) {
...
#ifdef CONFIG_KERNEL_MCS
        reply_t *reply =
            REPLY_PTR(thread_state_get_replyObject(thread->tcbState));
        if (reply != NULL) {
...
#ifdef CONFIG_KERNEL_MCS
        /* senders do not have reply objects in their state, and we are only
         * cancelling sends */
        assert(REPLY_PTR(thread_state_get_replyObject(thread->tcbState)) ==
               NULL);
        if (b == badge) {
          if (seL4_Fault_get_seL4_FaultType(thread->tcbFault) ==
              seL4_Fault_NullFault) {
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/interrupt.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
exception_t decodeIRQControlInvocation(word_t invLabel, word_t length,
  ...
  if (invLabel == IRQIssueIRQHandler) {
    ...
    if (isIRQActive(irq)) {
      current_syscall_error.type = seL4_RevokeFirst;
      userError("Rejecting request for IRQ %u. Already active.",
                (int)IRQT_TO_IRQ(irq));
      return EXCEPTION_SYSCALL_ERROR;
    }
...
exception_t invokeIRQControl(irq_t irq, cte_t *handlerSlot,
                             cte_t *controlSlot) {
  setIRQState(IRQSignal, irq);
  cteInsert(cap_irq_handler_cap_new(IRQT_TO_IDX(irq)), controlSlot,
            handlerSlot);
  ...
  switch (invLabel) {
    ...
    case IRQSetIRQHandler: {
      ...
      if (cap_get_capType(ntfnCap) != cap_notification_cap ||
        ...
        if (cap_get_capType(ntfnCap) != cap_notification_cap) {
          userError(
              "IRQSetHandler: provided cap is not an notification capability.");
        } else {
          userError(
              "IRQSetHandler: caller does not have send rights on the "
              "endpoint.");
        }
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
...
void deletingIRQHandler(irq_t irq) {
  ...
  /** GHOSTUPD: "(True, gs_set_assn cteDeleteOne_'proc (ucast
   * cap_notification_cap))" */
  cteDeleteOne(slot);
}

void deletedIRQHandler(irq_t irq) { setIRQState(IRQInactive, irq); }
...
#if defined ENABLE_SMP_SUPPORT && defined CONFIG_ARCH_ARM
  if (IRQ_IS_PPI(irq) && IRQT_TO_CORE(irq) != getCurrentCPUIndex()) {
    doRemoteMaskPrivateInterrupt(IRQT_TO_CORE(irq), irqState == IRQInactive,
                                 IRQT_TO_IDX(irq));
    return;
  }
#endif
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/notification.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline void ntfn_ptr_set_queue(notification_t *ntfnPtr,
                                      tcb_queue_t ntfn_queue) {
...
#ifdef CONFIG_KERNEL_MCS
static inline void maybeDonateSchedContext(tcb_t *tcb,
                                           notification_t *ntfnPtr) {
  if (tcb->tcbSchedContext == NULL) {
    sched_context_t *sc =
        SC_PTR(notification_ptr_get_ntfnSchedContext(ntfnPtr));
    if (sc != NULL && sc->scTcb == NULL) {
...
void sendSignal(notification_t *ntfnPtr, word_t badge) {
  switch (notification_ptr_get_state(ntfnPtr)) {
    case NtfnState_Idle: {
      ...
      if (tcb) {
        if (thread_state_ptr_get_tsType(&tcb->tcbState) ==
          ...
          setThreadState(tcb, ThreadState_Running);
          setRegister(tcb, badgeRegister, badge);
          MCS_DO_IF_SC(tcb, ntfnPtr, { possibleSwitchTo(tcb); })
#ifdef CONFIG_KERNEL_MCS
...
#endif
#ifdef CONFIG_VTX
        } else if (thread_state_ptr_get_tsType(&tcb->tcbState) ==
                   ThreadState_RunningVM) {
#ifdef ENABLE_SMP_SUPPORT
    ...
    case NtfnState_Waiting: {
      ...
      setThreadState(dest, ThreadState_Running);
      setRegister(dest, badgeRegister, badge);
      MCS_DO_IF_SC(dest, ntfnPtr, { possibleSwitchTo(dest); })
...
void receiveSignal(tcb_t *thread, cap_t cap, bool_t isBlocking) {
  ...
  switch (notification_ptr_get_state(ntfnPtr)) {
    ...
    case NtfnState_Active:
      setRegister(thread, badgeRegister,
                  notification_ptr_get_ntfnMsgIdentifier(ntfnPtr));
      notification_ptr_set_state(ntfnPtr, NtfnState_Idle);
#ifdef CONFIG_KERNEL_MCS
      maybeDonateSchedContext(thread, ntfnPtr);
      // If the SC has been donated to the current thread (in a reply_recv,
      // send_recv scenario) then we may need to perform refill_unblock_check if
      // the SC is becoming activated.
      if (thread->tcbSchedContext != NODE_STATE(ksCurSC) &&
...
void cancelSignal(tcb_t *threadPtr, notification_t *ntfnPtr) {
  ...
  /* Haskell error "cancelSignal: notification object must be in a waiting"
   * state */
...
static inline void doUnbindNotification(notification_t *ntfnPtr,
                                        tcb_t *tcbptr) {
...
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/objecttype.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
finaliseCap_ret_t finaliseCap(cap_t cap, bool_t final, bool_t exposed) {
    ...
    case cap_notification_cap:
      if (final) {
        ...
        schedContext_unbindNtfn(
            SC_PTR(notification_ptr_get_ntfnSchedContext(ntfn)));
#endif
  ...
  switch (cap_get_capType(cap)) {
    case cap_cnode_cap: {
      if (final) {
        fc_ret.remainder =
            Zombie_new(1ul << cap_cnode_cap_get_capCNodeRadix(cap),
    ...
    case cap_thread_cap: {
      if (final) {
        ...
        fc_ret.remainder = Zombie_new(tcbArchCNodeEntries, ZombieType_ZombieTCB,
                                      CTE_REF(cte_ptr));
        fc_ret.cleanupInfo = cap_null_cap_new();
        return fc_ret;
      }
...
cap_t createObject(object_t t, void *regionBase, word_t userSize,
                   bool_t deviceMemory) {
  ...
  switch ((api_object_t)t) {
    ...
    case seL4_UntypedObject:
       ...
       */
      return cap_untyped_cap_new(0, !!deviceMemory, userSize,
                                 WORD_REF(regionBase));
          ...
          "(True,
            ptr_arr_retyps (refills_len (unat \<acute>userSize)
                           ...
                           (Ptr ((ptr_val \<acute>regionBase) +
                                 word_of_nat (size_of TYPE(sched_context_C))) ::
         refill_C ptr)
            \<circ> ptr_retyp (Ptr (ptr_val \<acute>regionBase) ::
         sched_context_C ptr))" */
      /** GHOSTUPD: "(True, gs_new_sc_size (ptr_val \<acute>regionBase) (unat
       * \<acute>userSize))" */
      return cap_sched_context_cap_new(SC_REF(regionBase), userSize);

    case seL4_ReplyObject:
      /** AUXUPD: "(True, ptr_retyp (Ptr (ptr_val \<acute>regionBase) :: reply_C
       * ptr))" */
...
void createNewObjects(object_t t, cte_t *parent, cte_t *destCNode,
                      word_t destOffset, word_t destLength, void *regionBase,
                      word_t userSize, bool_t deviceMemory) {
  word_t objectSize;
  void *nextFreeArea;
  word_t i;
  ...
  for (i = 0; i < destLength; i++) {
    /* Create the object. */
    /** AUXUPD: "(True, typ_region_bytes (ptr_val \<acute> nextFreeArea +
     * ((\<acute> i) << unat (\<acute> objectSize))) (unat (\<acute>
     * objectSize)))" */
    cap_t cap =
        createObject(t, (void *)((word_t)nextFreeArea + (i << objectSize)),
...
#ifdef CONFIG_KERNEL_MCS
exception_t decodeInvocation(word_t invLabel, word_t length, cptr_t capIndex,
                             cte_t *slot, cap_t cap, bool_t block, bool_t call,
                             bool_t canDonate, bool_t firstPhase,
                             word_t *buffer)
#else
exception_t decodeInvocation(word_t invLabel, word_t length, cptr_t capIndex,
...
{
  if (isArchCap(cap)) {
    return Arch_decodeInvocation(invLabel, length, capIndex, slot, cap, call,
                                 buffer);
  }
  ...
  switch (cap_get_capType(cap)) {
    ...
    case cap_reply_cap:
      ...
      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
      return performInvocation_Reply(TCB_PTR(cap_reply_cap_get_capTCBPtr(cap)),
                                     slot,
                                     cap_reply_cap_get_capReplyCanGrant(cap));
    ...
    case cap_thread_cap:
      ...
      if (unlikely(firstPhase)) {
        userError(
            "Cannot invoke thread capabilities in the first phase of an "
            "invocation");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
        return EXCEPTION_SYSCALL_ERROR;
    ...
    case cap_domain_cap:
      ...
      if (unlikely(firstPhase)) {
        userError(
            "Cannot invoke domain capabilities in the first phase of an "
            "invocation");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
        return EXCEPTION_SYSCALL_ERROR;
    ...
    case cap_cnode_cap:
      ...
      if (unlikely(firstPhase)) {
        userError(
            "Cannot invoke cnode capabilities in the first phase of an "
            "invocation");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
        return EXCEPTION_SYSCALL_ERROR;
    ...
    case cap_irq_handler_cap:
      return decodeIRQHandlerInvocation(
          invLabel, IDX_TO_IRQT(cap_irq_handler_cap_get_capIRQ(cap)));

#ifdef CONFIG_KERNEL_MCS
    case cap_sched_control_cap:
      if (unlikely(firstPhase)) {
        userError(
            "Cannot invoke sched control capabilities in the first phase of an "
            "invocation");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
        return EXCEPTION_SYSCALL_ERROR;
    ...
    case cap_sched_context_cap:
      if (unlikely(firstPhase)) {
        userError(
            "Cannot invoke sched context capabilities in the first phase of an "
            "invocation");
        current_syscall_error.type = seL4_InvalidCapability;
        current_syscall_error.invalidCapNumber = 0;
        return EXCEPTION_SYSCALL_ERROR;
...
exception_t performInvocation_Endpoint(endpoint_t *ep, word_t badge,
                                       ...
                                       bool_t canDonate) {
  sendIPC(block, call, badge, canGrant, canGrantReply, canDonate,
          NODE_STATE(ksCurThread), ep);
...
exception_t performInvocation_Endpoint(endpoint_t *ep, word_t badge,
                                       bool_t canGrant, bool_t canGrantReply,
                                       bool_t block, bool_t call) {
  sendIPC(block, call, badge, canGrant, canGrantReply, NODE_STATE(ksCurThread),
          ep);
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/tcb.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline void addToBitmap(word_t cpu, word_t dom, word_t prio) {
  ...
  NODE_STATE_ON_CORE(ksReadyQueuesL2Bitmap[dom][l1index_inverted], cpu) |=
...
static inline void removeFromBitmap(word_t cpu, word_t dom, word_t prio) {
  ...
  NODE_STATE_ON_CORE(ksReadyQueuesL2Bitmap[dom][l1index_inverted], cpu) &=
      ~BIT(prio & MASK(wordRadix));
  if (unlikely(!NODE_STATE_ON_CORE(ksReadyQueuesL2Bitmap[dom][l1index_inverted],
                                   cpu))) {
    NODE_STATE_ON_CORE(ksReadyQueuesL1Bitmap[dom], cpu) &= ~BIT(l1index);
  }
}
...
void tcbDebugAppend(tcb_t *tcb) {
  ...
  if (NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity)) {
    TCB_PTR_DEBUG_PTR(NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity))
        ->tcbDebugPrev = tcb;
  }
...
void tcbDebugRemove(tcb_t *tcb) {
  ...
  if (tcb == NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity)) {
    NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity) =
        TCB_PTR_DEBUG_PTR(NODE_STATE_ON_CORE(ksDebugTCBs, tcb->tcbAffinity))
            ->tcbDebugNext;
  } else {
    assert(TCB_PTR_DEBUG_PTR(tcb)->tcbDebugPrev);
    TCB_PTR_DEBUG_PTR(debug_tcb->tcbDebugPrev)->tcbDebugNext =
        debug_tcb->tcbDebugNext;
  }

  if (debug_tcb->tcbDebugNext) {
    TCB_PTR_DEBUG_PTR(debug_tcb->tcbDebugNext)->tcbDebugPrev =
        debug_tcb->tcbDebugPrev;
  }
...
void tcbReleaseEnqueue(tcb_t *tcb) {
  ...
  while (after != NULL && refill_head(tcb->tcbSchedContext)->rTime >=
                              refill_head(after->tcbSchedContext)->rTime) {
    before = after;
...
tcb_t *tcbReleaseDequeue(void) {
  ...
  SMP_COND_STATEMENT(
      assert(NODE_STATE(ksReleaseHead)->tcbAffinity == getCurrentCPUIndex()));
...
void setupCallerCap(tcb_t *sender, tcb_t *receiver, bool_t canGrant) {
  ...
  cteInsert(cap_reply_cap_new(canGrant, false, TCB_REF(sender)), replySlot,
            callerSlot);
}
...
exception_t lookupExtraCaps(tcb_t *thread, word_t *bufferPtr,
                            seL4_MessageInfo_t info) {
  lookupSlot_raw_ret_t lu_ret;
  cptr_t cptr;
  word_t i, length;
...
word_t copyMRs(tcb_t *sender, word_t *sendBuf, tcb_t *receiver, word_t *recvBuf,
               word_t n) {
  word_t i;
...
#ifdef ENABLE_SMP_SUPPORT
/* This checks if the current updated to scheduler queue is changing the
 * previous scheduling decision made by the scheduler. If its a case, an
 * `irq_reschedule_ipi` is sent */
void remoteQueueUpdate(tcb_t *tcb) {
  ...
  if (tcb->tcbAffinity != getCurrentCPUIndex() &&
    ...
    /* reschedule if the target core is idle or we are waking a higher priority
     * thread (or if a new irq would need to be set on MCS) */
...
#ifdef CONFIG_HARDWARE_DEBUG_API
static exception_t invokeConfigureSingleStepping(bool_t call, word_t *buffer,
                                                 tcb_t *t, uint16_t bp_num,
                                                 word_t n_instrs) {
...
static exception_t decodeConfigureSingleStepping(cap_t cap, bool_t call,
                                                 word_t *buffer) {
  uint16_t bp_num;
  word_t n_instrs;
  tcb_t *tcb;
...
static exception_t invokeSetBreakpoint(tcb_t *tcb, uint16_t bp_num,
                                       word_t vaddr, word_t type, word_t size,
                                       word_t rw) {
...
static exception_t decodeSetBreakpoint(cap_t cap, word_t *buffer) {
  ...
  if (vaddr >= (word_t)USER_TOP) {
    userError(
        "Debug: Invalid address %lx: bp addresses must be userspace "
        "addresses.",
        vaddr);
    current_syscall_error.type = seL4_InvalidArgument;
    current_syscall_error.invalidArgumentNumber = 1;
  ...
  if (type != seL4_InstructionBreakpoint && type != seL4_DataBreakpoint) {
    ...
    if ((seL4_FirstWatchpoint == -1 || bp_num >= seL4_FirstWatchpoint) &&
      ...
      userError(
          "Debug: Can't specify a watchpoint ID with type "
    ...
    if (seL4_FirstWatchpoint != -1 && bp_num < seL4_FirstWatchpoint) {
      userError(
          "Debug: Data watchpoints cannot specify non-data watchpoint ID.");
      current_syscall_error.type = seL4_InvalidArgument;
      current_syscall_error.invalidArgumentNumber = 2;
      return EXCEPTION_SYSCALL_ERROR;
    }
    ...
    userError(
        "Debug: Use a software breakpoint instruction to trigger a "
        "software breakpoint.");
    current_syscall_error.type = seL4_InvalidArgument;
    current_syscall_error.invalidArgumentNumber = 2;
    return EXCEPTION_SYSCALL_ERROR;
  ...
  if (size > 0 && vaddr & (size - 1)) {
     ...
     */
    userError(
        "Debug: Unaligned data watchpoint address %lx (size %lx) "
        "rejected.\n",
        vaddr, size);
  ...
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return invokeSetBreakpoint(tcb, bp_num, vaddr, type, size, rw);
}

static exception_t invokeGetBreakpoint(bool_t call, word_t *buffer, tcb_t *tcb,
                                       uint16_t bp_num) {
  tcb_t *thread;
  thread = NODE_STATE(ksCurThread);
  getBreakpoint_t res;
...
static exception_t invokeSetTLSBase(tcb_t *thread, word_t tls_base) {
  ...
  if (thread == NODE_STATE(ksCurThread)) {
    /* If this is the current thread force a reschedule to ensure that any
     * changes to the TLS_BASE are realized */
    rescheduleRequired();
  }
...
exception_t decodeTCBInvocation(word_t invLabel, word_t length, cap_t cap,
  ...
  /* Stall the core if we are operating on a remote TCB that is currently
   * running */
  SMP_COND_STATEMENT(
      remoteTCBStall(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)));)
  ...
  switch (invLabel) {
    ...
    case TCBSuspend:
      ...
      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
      return invokeTCB_Suspend(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)));

    case TCBResume:
      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
      return invokeTCB_Resume(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)));
    ...
    case TCBSetAffinity:
      ...
      /* There is no notion of arch specific TCB invocations so this needs to go
       * here */
#ifdef CONFIG_VTX
    case TCBSetEPTRoot:
...
exception_t decodeCopyRegisters(cap_t cap, word_t length, word_t *buffer) {
  ...
  return invokeTCB_CopyRegisters(
      ...
      flags & BIT(CopyRegisters_transferInteger), transferArch);
}
...
exception_t decodeReadRegisters(cap_t cap, word_t length, bool_t call,
  ...
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return invokeTCB_ReadRegisters(TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)),
                                 flags & BIT(ReadRegisters_suspend), n,
                                 transferArch, call);
}
...
exception_t decodeWriteRegisters(cap_t cap, word_t length, word_t *buffer) {
  ...
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return invokeTCB_WriteRegisters(thread, flags & BIT(WriteRegisters_resume), w,
                                  transferArch, buffer);
}
...
#define TCBCONFIGURE_ARGS 4
  ...
  return invokeTCB_ThreadControlCaps(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, cap_null_cap_new(),
      NULL, cap_null_cap_new(), NULL, cRootCap, cRootSlot, vRootCap, vRootSlot,
      bufferAddr, bufferCap, bufferSlot,
      thread_control_caps_update_space | thread_control_caps_update_ipc_buffer);
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, faultEP, NULL_PRIO,
      NULL_PRIO, cRootCap, cRootSlot, vRootCap, vRootSlot, bufferAddr,
      bufferCap, bufferSlot,
...
exception_t decodeSetPriority(cap_t cap, word_t length, word_t *buffer) {
  ...
  return invokeTCB_ThreadControlSched(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, cap_null_cap_new(),
      NULL, NULL_PRIO, newPrio, NULL, thread_control_sched_update_priority);
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, 0, NULL_PRIO, newPrio,
      cap_null_cap_new(), NULL, cap_null_cap_new(), NULL, 0, cap_null_cap_new(),
...
exception_t decodeSetMCPriority(cap_t cap, word_t length, word_t *buffer) {
  ...
  if (status != EXCEPTION_NONE) {
    userError(
        "TCB SetMCPriority: Requested maximum controlled priority %lu too high "
        "(max %lu).",
  ...
  return invokeTCB_ThreadControlSched(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, cap_null_cap_new(),
      NULL, newMcp, NULL_PRIO, NULL, thread_control_sched_update_mcp);
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, 0, newMcp, NULL_PRIO,
      cap_null_cap_new(), NULL, cap_null_cap_new(), NULL, 0, cap_null_cap_new(),
...
exception_t decodeSetTimeoutEndpoint(cap_t cap, cte_t *slot) {
  ...
  return invokeTCB_ThreadControlCaps(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, cap_null_cap_new(),
      NULL, thCap, thSlot, cap_null_cap_new(), NULL, cap_null_cap_new(), NULL,
      0, cap_null_cap_new(), NULL, thread_control_caps_update_timeout);
}
#endif

#ifdef CONFIG_KERNEL_MCS
exception_t decodeSetSchedParams(cap_t cap, word_t length, cte_t *slot,
                                 word_t *buffer)
#else
  ...
  switch (cap_get_capType(scCap)) {
    case cap_sched_context_cap:
      ...
      if (isBlocked(tcb) && !sc_released(sc)) {
        userError(
            "TCB Configure: tcb blocked and scheduling context not "
            "schedulable.");
        current_syscall_error.type = seL4_IllegalOperation;
        return EXCEPTION_SYSCALL_ERROR;
      }
    ...
    case cap_null_cap:
      if (tcb == NODE_STATE(ksCurThread)) {
        userError(
            "TCB SetSchedParams: Cannot change sched_context of current "
            "thread");
        current_syscall_error.type = seL4_IllegalOperation;
        return EXCEPTION_SYSCALL_ERROR;
      }
...
#ifdef CONFIG_KERNEL_MCS
  return invokeTCB_ThreadControlSched(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, fhCap, fhSlot, newMcp,
      newPrio, sc,
...
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), NULL, 0, newMcp, newPrio,
      cap_null_cap_new(), NULL, cap_null_cap_new(), NULL, 0, cap_null_cap_new(),
...
exception_t decodeSetIPCBuffer(cap_t cap, word_t length, cte_t *slot,
  ...
  return invokeTCB_ThreadControlCaps(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, cap_null_cap_new(),
      NULL, cap_null_cap_new(), NULL, cap_null_cap_new(), NULL,
      cap_null_cap_new(), NULL, cptr_bufferPtr, bufferCap, bufferSlot,
      thread_control_caps_update_ipc_buffer);
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, 0, NULL_PRIO, NULL_PRIO,
      cap_null_cap_new(), NULL, cap_null_cap_new(), NULL, cptr_bufferPtr,
      bufferCap, bufferSlot, thread_control_update_ipc_buffer);
...
exception_t decodeSetSpace(cap_t cap, word_t length, cte_t *slot,
  ...
  if (length < DECODE_SET_SPACE_PARAMS ||
      current_extra_caps.excaprefs[0] == NULL ||
      current_extra_caps.excaprefs[1] == NULL
#ifdef CONFIG_KERNEL_MCS
      || current_extra_caps.excaprefs[2] == NULL
...
#ifdef CONFIG_KERNEL_MCS
  return invokeTCB_ThreadControlCaps(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, fhCap, fhSlot,
      cap_null_cap_new(), NULL, cRootCap, cRootSlot, vRootCap, vRootSlot, 0,
      cap_null_cap_new(), NULL,
      thread_control_caps_update_space | thread_control_caps_update_fault);
#else
  return invokeTCB_ThreadControl(
      TCB_PTR(cap_thread_cap_get_capTCBPtr(cap)), slot, faultEP, NULL_PRIO,
      NULL_PRIO, cRootCap, cRootSlot, vRootCap, vRootSlot, 0,
...
exception_t decodeDomainInvocation(word_t invLabel, word_t length,
  ...
  if (unlikely(length == 0)) {
    ...
    if (domain >= numDomains) {
      userError("Domain Configure: invalid domain (%lu >= %u).", domain,
                numDomains);
      current_syscall_error.type = seL4_InvalidArgument;
      current_syscall_error.invalidArgumentNumber = 0;
      return EXCEPTION_SYSCALL_ERROR;
...
static inline exception_t installTCBCap(tcb_t *target, cap_t tCap, cte_t *slot,
                                        tcb_cnode_index_t index, cap_t newCap,
                                        cte_t *srcSlot) {
  ...
  if (sameObjectAs(newCap, srcSlot->cap) && sameObjectAs(tCap, slot->cap)) {
...
#ifdef CONFIG_KERNEL_MCS
exception_t invokeTCB_ThreadControlCaps(
    tcb_t *target, cte_t *slot, cap_t fh_newCap, cte_t *fh_srcSlot,
    cap_t th_newCap, cte_t *th_srcSlot, cap_t cRoot_newCap,
    cte_t *cRoot_srcSlot, cap_t vRoot_newCap, cte_t *vRoot_srcSlot,
    word_t bufferAddr, cap_t bufferCap, cte_t *bufferSrcSlot,
    thread_control_flag_t updateFlags) {
  exception_t e;
  cap_t tCap = cap_thread_cap_new((word_t)target);

  if (updateFlags & thread_control_caps_update_fault) {
    e = installTCBCap(target, tCap, slot, tcbFaultHandler, fh_newCap,
                      fh_srcSlot);
    if (e != EXCEPTION_NONE) {
      return e;
    }
  ...
  if (updateFlags & thread_control_caps_update_timeout) {
    e = installTCBCap(target, tCap, slot, tcbTimeoutHandler, th_newCap,
                      th_srcSlot);
    if (e != EXCEPTION_NONE) {
      return e;
    }
  ...
  if (updateFlags & thread_control_caps_update_space) {
    e = installTCBCap(target, tCap, slot, tcbCTable, cRoot_newCap,
                      cRoot_srcSlot);
    if (e != EXCEPTION_NONE) {
      return e;
    }

    e = installTCBCap(target, tCap, slot, tcbVTable, vRoot_newCap,
                      vRoot_srcSlot);
    if (e != EXCEPTION_NONE) {
      return e;
    }
...
exception_t invokeTCB_ThreadControlSched(tcb_t *target, cte_t *slot,
  ...
  if (updateFlags & thread_control_sched_update_fault) {
    cap_t tCap = cap_thread_cap_new((word_t)target);
    exception_t e = installTCBCap(target, tCap, slot, tcbFaultHandler,
                                  fh_newCap, fh_srcSlot);
    if (e != EXCEPTION_NONE) {
      return e;
    }
  }
...
exception_t invokeTCB_CopyRegisters(tcb_t *dest, tcb_t *tcb_src,
                                    ...
                                    bool_t transferFrame,
                                    bool_t transferInteger,
                                    word_t transferArch) {
  if (suspendSource) {
    suspend(tcb_src);
  }
...
exception_t invokeTCB_WriteRegisters(tcb_t *dest, bool_t resumeTarget, word_t n,
                                     word_t arch, word_t *buffer) {
  word_t i;
  word_t pc;
  exception_t e;
  bool_t archInfo;
  ...
  for (i = 0; i < n_gpRegisters && i + n_frameRegisters < n; i++) {
    setRegister(dest, gpRegisters[i],
                sanitiseRegister(
                    gpRegisters[i],
                    getSyscallArg(i + n_frameRegisters + 2, buffer), archInfo));
  }
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/object/untyped.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
exception_t decodeUntypedInvocation(word_t invLabel, word_t length, cte_t *slot,
  ...
  /* Exclude impossibly large object sizes. getObjectSize can overflow if
     userObjSize is close to 2^wordBits, which is nonsensical in any case, so we
     check that this did not happen. userObjSize will always need to be less
     than wordBits. */
...
#ifdef CONFIG_KERNEL_MCS
  if (newType == seL4_SchedContextObject &&
      userObjSize < seL4_MinSchedContextBits) {
  ...
  if (nodeWindow > nodeSize - nodeOffset) {
    userError(
        "Untyped Retype: Requested destination window overruns size of node.");
    current_syscall_error.type = seL4_RangeError;
    current_syscall_error.rangeErrorMin = 1;
  ...
  if ((untypedFreeBytes >> objectSize) < nodeWindow) {
    userError(
        ...
        (word_t)nodeWindow, (objectSize >= wordBits ? -1 : (1ul << objectSize)),
  ...
  /* Perform the retype. */
  setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
  return invokeUntyped_Retype(slot, reset, (void *)alignedFreeRef, newType,
                              userObjSize, destCNode, nodeOffset, nodeWindow,
                              deviceMemory);
}
...
static exception_t resetUntypedCap(cte_t *srcSlot) {
  ...
  word_t offset =
      FREE_INDEX_TO_OFFSET(cap_untyped_cap_get_capFreeIndex(prev_cap));
  exception_t status;
...
exception_t invokeUntyped_Retype(cte_t *srcSlot, bool_t reset, void *retypeBase,
                                 object_t newType, word_t userSize,
                                 cte_t *destCNode, word_t destOffset,
  ...
  srcSlot->cap = cap_untyped_cap_set_capFreeIndex(
      srcSlot->cap, GET_FREE_INDEX(regionBase, freeRef));
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/acpi.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
enum acpi_type { ACPI_RSDP, ACPI_RSDT };
...
compile_assert(acpi_dmar_rmrr_packed,
               sizeof(acpi_dmar_rmrr_t) == sizeof(acpi_dmar_header_t) + 20 +
                                               sizeof(acpi_dmar_devscope_t))
...
unverified_compile_assert(acpi_madt_iso_packed,
                          OFFSETOF(acpi_madt_iso_t, flags) ==
                              sizeof(acpi_madt_header_t) + 6)
...
BOOT_CODE static void *acpi_table_init(void *entry, enum acpi_type table_type) {
  ...
  switch (table_type) {
    case ACPI_RSDP: {
      ...
      pages_for_table =
          (rsdp_entry->length + offset_in_page) / MASK(LARGE_PAGE_BITS) + 1;
      break;
    }
    case ACPI_RSDT: {  // RSDT, MADT, DMAR etc.
      acpi_rsdt_t *rsdt_entry = (acpi_rsdt_t *)entry;
      pages_for_table =
          (rsdt_entry->header.length + offset_in_page) / MASK(LARGE_PAGE_BITS) +
          1;
      break;
    }
    default:
...
BOOT_CODE bool_t acpi_validate_rsdp(acpi_rsdp_t *acpi_rsdp) {
  ...
  if (acpi_rsdp->revision > 0 &&
      acpi_calc_checksum((char *)acpi_rsdp, sizeof(*acpi_rsdp)) != 0) {
    printf("BIOS: ACPIv2 information corrupt\n");
    return false;
  }

  /* verify the rsdt, even though we do not actually make use of the mapping
   * right now */
  ...
  if (acpi_calc_checksum((char *)acpi_rsdt_mapped,
                         acpi_rsdt_mapped->header.length) != 0) {
...
BOOT_CODE uint32_t acpi_madt_scan(acpi_rsdp_t *acpi_rsdp, cpu_id_t *cpu_list,
                                  uint32_t *num_ioapic,
                                  paddr_t *ioapic_paddrs) {
  ...
  acpi_rsdt_mapped = (acpi_rsdt_t *)acpi_table_init(
      (acpi_rsdt_t *)(word_t)acpi_rsdp->rsdt_address, ACPI_RSDT);

  num_cpu = 0;
  *num_ioapic = 0;

  assert(acpi_rsdt_mapped->header.length >= sizeof(acpi_header_t));
  /* Divide by uint32_t explicitly as this is the size as mandated by the ACPI
   * standard */
  entries = (acpi_rsdt_mapped->header.length - sizeof(acpi_header_t)) /
            sizeof(uint32_t);
  for (count = 0; count < entries; count++) {
    ...
    if (strncmp(acpi_str_apic, acpi_madt_mapped->header.signature, 4) == 0) {
      ...
      while ((char *)acpi_madt_header <
             (char *)acpi_madt_mapped + acpi_madt_mapped->header.length) {
        switch (acpi_madt_header->type) {
          /* ACPI specifies the following rules when listing APIC IDs:
           *  - Boot processor is listed first
           *  - For multi-threaded processors, BIOS should list the first
           * logical processor of each of the individual multi-threaded
           * processors in MADT before listing any of the second logical
           * processors.
           *  - APIC IDs < 0xFF should be listed in APIC subtable, APIC IDs >=
           * 0xFF should be listed in X2APIC subtable */
          case MADT_APIC: {
          ...
          case MADT_x2APIC: {
            uint32_t cpu_id =
                ((acpi_madt_x2apic_t *)acpi_madt_header)->x2apic_id;
          ...
          case MADT_IOAPIC:
            printf("ACPI: MADT_IOAPIC ioapic_id=%d ioapic_addr=0x%x gsib=%d\n",
        ...
        acpi_madt_header = (acpi_madt_header_t *)((char *)acpi_madt_header +
...
BOOT_CODE bool_t acpi_fadt_scan(acpi_rsdp_t *acpi_rsdp) {
  unsigned int entries;
  uint32_t count;
...
BOOT_CODE void acpi_dmar_scan(acpi_rsdp_t *acpi_rsdp, paddr_t *drhu_list,
                              uint32_t *num_drhu, uint32_t max_drhu_list_len,
  ...
  *num_drhu = 0;
  ...
  for (count = 0; count < entries; count++) {
    ...
    if (strncmp(acpi_str_dmar, acpi_dmar_mapped->header.signature, 4) == 0) {
      ...
      while ((char *)acpi_dmar_header <
             (char *)acpi_dmar_mapped + acpi_dmar_mapped->header.length) {
        switch (acpi_dmar_header->type) {
          case DMAR_DRHD:
            ...
            if (reg_baseh != 0) {
              printf(
                  "ACPI: DMAR_DRHD reg_base exceeds 32 bit, disabling IOMMU "
                  "support\n");
          ...
          case DMAR_RMRR:
            ...
            for (i = 0;
                 i <= (acpi_dmar_header->length - sizeof(acpi_dmar_rmrr_t)) /
                          sizeof(acpi_dmar_devscope_t);
                 i++) {
              ...
              if (acpi_dmar_devscope->type != 1) {
                ...
                printf(
                    "ACPI: RMRR device scope: non-PCI-Endpoint-Devices not "
                    "supported yet, disabling IOMMU support\n");
                *num_drhu = 0; /* report zero IOMMUs */
                return;
              }
              ...
              if (acpi_dmar_devscope->length > sizeof(acpi_dmar_devscope_t)) {
                /* FIXME - bugzilla bug 170 */
                printf(
                    "ACPI: RMRR device scope: devices behind bridges not "
                    "supported yet, disabling IOMMU support\n");
                *num_drhu = 0; /* report zero IOMMUs */
                return;
              }

              dev_id = get_dev_id(acpi_dmar_devscope->start_bus,
                                  acpi_dmar_devscope->path_0.dev,
                                  acpi_dmar_devscope->path_0.fun);

              if (rmrr_count == CONFIG_MAX_RMRR_ENTRIES) {
                printf(
                    "ACPI: Too many RMRR entries, disabling IOMMU support\n");
                *num_drhu = 0;
                return;
              }
              printf(
                  "\tACPI: registering RMRR entry for region for device: "
                  "bus=0x%x dev=0x%x fun=0x%x\n",
                  acpi_dmar_devscope->start_bus, acpi_dmar_devscope->path_0.dev,
                  acpi_dmar_devscope->path_0.fun);

              rmrr_list->entries[rmrr_count].device = dev_id;
              rmrr_list->entries[rmrr_count].base = acpi_dmar_rmrr->reg_base[0];
              rmrr_list->entries[rmrr_count].limit =
                  acpi_dmar_rmrr->reg_limit[0];
              rmrr_count++;
            }
            break;
        ...
        acpi_dmar_header = (acpi_dmar_header_t *)((char *)acpi_dmar_header +
                                                  acpi_dmar_header->length);
      }
    }
  }
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/hardware.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
BOOT_CODE bool_t platAddDevices(void) {
  ...
  if (CONFIG_PADDR_USER_DEVICE_TOP > 0xFFFFFFF8) {
    if (!reserve_region(
            (p_region_t){(word_t)0xFFFFFFF8, (word_t)0xFFFFFFF8 + 8})) {
      return false;
    }
  }
  return true;
...
BOOT_CODE uint32_t tsc_init(void) {
  ...
  uint32_t valid_models[] = {
      ...
      SANDY_BRIDGE_1_MODEL_ID, SANDY_BRIDGE_2_MODEL_ID, HASWELL_1_MODEL_ID,
      HASWELL_2_MODEL_ID, HASWELL_3_MODEL_ID, HASWELL_4_MODEL_ID,
      IVY_BRIDGE_1_MODEL_ID, IVY_BRIDGE_2_MODEL_ID, IVY_BRIDGE_3_MODEL_ID,
      /* BROADWELL_1_MODEL_ID is an Atom that doesn't support the MSR */
      BROADWELL_2_MODEL_ID, BROADWELL_3_MODEL_ID, BROADWELL_4_MODEL_ID,
      BROADWELL_5_MODEL_ID, SKYLAKE_1_MODEL_ID, SKYLAKE_2_MODEL_ID};
  ...
  if (model_info->family == IA32_PREFETCHER_COMPATIBLE_FAMILIES_ID) {
    for (int i = 0; i < ARRAY_SIZE(valid_models); i++) {
      if (model_info->model == valid_models[i]) {
        /* read tsc freq from the platform info msr. Under some environments
         * such as KVM this MSR will cause a GP fault even though it should be
         * there. As such we perform a 'safe' rdmsr, which will catch a GP fault
         * and tells through .success whether or not one happened */
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/intel-vtd.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static inline uint32_t vtd_read32(drhu_id_t drhu_id, uint32_t offset) {
  return *(volatile uint32_t *)(PPTR_DRHU_START + (drhu_id << PAGE_BITS) +
                                offset);
}

static inline void vtd_write32(drhu_id_t drhu_id, uint32_t offset,
                               uint32_t value) {
  *(volatile uint32_t *)(PPTR_DRHU_START + (drhu_id << PAGE_BITS) + offset) =
      value;
}

static inline uint64_t vtd_read64(drhu_id_t drhu_id, uint32_t offset) {
  return *(volatile uint64_t *)(PPTR_DRHU_START + (drhu_id << PAGE_BITS) +
                                offset);
}

static inline void vtd_write64(drhu_id_t drhu_id, uint32_t offset,
                               uint64_t value) {
  *(volatile uint64_t *)(PPTR_DRHU_START + (drhu_id << PAGE_BITS) + offset) =
      value;
}
...
BOOT_CODE word_t vtd_get_n_paging(acpi_rmrr_list_t *rmrr_list) {
  ...
  for (word_t i = 1; i < rmrr_list->num; i++) {
    if (vtd_get_root_index(rmrr_list->entries[i].device) !=
        ...
        rmrr_list->entries[i].limit !=
            filtered.entries[filtered.num - 1].limit) {
  ...
  for (word_t i = x86KSnumIOPTLevels - 1; i > 0; i--) {
    ...
    if ((VTD_PT_INDEX_BITS * i + seL4_PageBits) >= 32) {
      ...
      for (word_t j = 0; j < filtered.num; j++) {
        v_region_t region = (v_region_t){.start = filtered.entries[j].base,
                                         .end = filtered.entries[j].limit};
        size +=
            get_n_paging(region, 32 - (VTD_PT_INDEX_BITS * i + seL4_PageBits));
      }
    }
  }
  return size;
 ...
 * in iospace.c
 */
BOOT_CODE static void vtd_map_reserved_page(vtd_cte_t *vtd_context_table,
                                            int context_index, paddr_t addr) {
  int i;
  vtd_pte_t *iopt;
  ...
  if (!vtd_cte_ptr_get_present(vtd_context_slot)) {
    ...
    *vtd_context_slot =
        vtd_cte_new(x86KSFirstValidIODomain, /* Domain ID */
                    true, /* RMRR Mapping                           */
  ...
  for (i = x86KSnumIOPTLevels - 1; i >= 0; i--) {
    ...
    if (VTD_PT_INDEX_BITS * i + seL4_PageBits >= 32) {
      ...
      iopt_index = ((addr >> seL4_PageBits) >> (VTD_PT_INDEX_BITS * i)) &
                   MASK(VTD_PT_INDEX_BITS);
    }
...
BOOT_CODE static void vtd_create_context_table(uint8_t bus,
                                               acpi_rmrr_list_t *rmrr_list) {
  word_t i;
  ...
  x86KSvtdRootTable[bus] =
      vtd_rte_new(pptr_to_paddr(vtd_context_table), /* Context Table Pointer */
                  true                              /* Present               */
      );
  ...
  for (i = 0; i < rmrr_list->num; i++) {
    if (vtd_get_root_index(rmrr_list->entries[i].device) == bus) {
      uint32_t addr;
      for (addr = rmrr_list->entries[i].base;
           addr < rmrr_list->entries[i].limit; addr += BIT(seL4_PageBits)) {
        vtd_map_reserved_page(
            vtd_context_table,
            vtd_get_context_index(rmrr_list->entries[i].device), addr);
      }
    }
  }
}
...
#endif /* CONFIG_IOMMU */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/io.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void serial_init(uint16_t port) {
  ...
  out8(port + 3,
       0x03); /* line control register: set 8 bit, no parity, 1 stop bit */
...
#endif /* CONFIG_DEBUG_BUILD */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/ioapic.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
static uint32_t
    ioredtbl_state[IOAPIC_IRQ_LINES * MAX(1, CONFIG_MAX_NUM_IOAPIC)];
...
static void ioapic_write(uint32_t ioapic, word_t reg, uint32_t value) {
  *(volatile uint32_t *)((word_t)(PPTR_IOAPIC_START + ioapic * BIT(PAGE_BITS)) +
                         reg) = value;
}

static uint32_t ioapic_read(uint32_t ioapic, word_t reg) {
  return *(volatile uint32_t *)((word_t)(PPTR_IOAPIC_START +
                                         ioapic * BIT(PAGE_BITS)) +
                                reg);
}
...
static void single_ioapic_init(word_t ioapic, cpu_id_t delivery_cpu) {
  ...
  for (i = 0; i < IOAPIC_IRQ_LINES; i++) {
    ...
    ioapic_write(ioapic, IOAPIC_WINDOW,
                 (ioapic_read(ioapic, IOAPIC_WINDOW) &
                  MASK(IOREDTBL_HIGH_RESERVED_BITS)) |
                     (delivery_cpu << IOREDTBL_HIGH_RESERVED_BITS));
...
exception_t ioapic_decode_map_pin_to_vector(word_t ioapic, word_t pin,
                                            word_t level, word_t polarity,
                                            word_t vector) {
  if (num_ioapics == 0) {
  ...
  if (ioapic >= num_ioapics) {
    userError("Invalid IOAPIC %ld, only have %ld", (long)ioapic,
              (long)num_ioapics);
    current_syscall_error.type = seL4_RangeError;
    current_syscall_error.rangeErrorMin = 0;
  ...
  if (pin >= IOAPIC_IRQ_LINES) {
    userError("Invalid IOAPIC pin %ld, there are %d pins", (long)pin,
              IOAPIC_IRQ_LINES);
    current_syscall_error.type = seL4_RangeError;
    current_syscall_error.rangeErrorMin = 0;
...
void ioapic_map_pin_to_vector(word_t ioapic, word_t pin, word_t level,
  ...
  ioredtbl_high =
      ioapic_read(ioapic, IOAPIC_WINDOW) & MASK(IOREDTBL_HIGH_RESERVED_BITS);
  ...
  ioredtbl_state[index] = IOREDTBL_LOW_INTERRUPT_MASK |
                          ...
                          (polarity << IOREDTBL_LOW_POLARITY_SHIFT) | vector;
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/pic.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
/* Program PIC (i8259) to remap IRQs 0-15 to interrupt vectors starting at
 * 'interrupt' */
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/plat/pc99/machine/pit.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/smp/ipi.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void ipiStallCoreCallback(bool_t irqPath) {
  if (clh_is_self_in_queue() && !irqPath) {
    /* The current thread is running as we would replace this thread with an
     * idle thread
     *
     * The instruction should be re-executed if we are in kernel to handle
     * syscalls. Also, thread in 'ThreadState_RunningVM' should remain in same
     * state. Note that, 'ThreadState_Restart' does not always result in
     * regenerating exception if we are in kernel to handle them, e.g. hardware
     * single step exception. */
    if (thread_state_ptr_get_tsType(&NODE_STATE(ksCurThread)->tcbState) ==
        ThreadState_Running) {
      setThreadState(NODE_STATE(ksCurThread), ThreadState_Restart);
    }
    ...
    while (big_kernel_lock.node_owners[getCurrentCPUIndex()].next->value !=
           CLHState_Granted) {
      if (clh_is_ipi_pending(getCurrentCPUIndex())) {
        /* Multiple calls for similar reason could result in stack overflow */
        assert((IpiRemoteCall_t)remoteCall != IpiRemoteCall_Stall);
        handleIPI(CORE_IRQ_TO_IRQT(getCurrentCPUIndex(), irq_remote_call_ipi),
                  irqPath);
      }
      arch_pause();
    }
    ...
    /* We get here either without grabbing the lock from normal interrupt path
     * or from inside the lock while waiting to grab the lock for handling
     * pending interrupt. In latter case, we return to the 'clh_lock_acquire' to
     * grab the lock and handle the pending interrupt. Its valid as interrups
     * are async events! */
    SCHED_ENQUEUE_CURRENT_TCB;
    switchToIdleThread();
#ifdef CONFIG_KERNEL_MCS
...
void doRemoteMaskOp(IpiRemoteCall_t func, word_t data1, word_t data2,
                    word_t data3, word_t mask) {
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/smp/lock.c"
/*
 * Copyright 2020, Data61, CSIRO (ABN 41 687 119 230)
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#endif /* ENABLE_SMP_SUPPORT */
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/string.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/kernel/src/util.c"
/*
 * Copyright 2014, General Dynamics C4 Systems
 *
 * SPDX-License-Identifier: GPL-2.0-only
...
void memzero(void *s, unsigned long n) {
  ...
  /* We will never memzero an area larger than the largest current
     live object */
  /** GHOSTUPD: "(gs_get_assn cap_get_capSizeBits_'proc \<acute>ghost'state = 0
      \<or> \<acute>n <= gs_get_assn cap_get_capSizeBits_'proc
     \<acute>ghost'state, id)" */
...
void *VISIBLE memset(void *s, unsigned long c, unsigned long n) {
   ...
   */
  if (likely(c == 0 && ((unsigned long)s % sizeof(unsigned long)) == 0 &&
...
compile_assert(clz_ulong_32_or_64,
...
#ifdef CONFIG_CLZ_32
CONST int __clzsi2(uint32_t x) { return clz32(x); }
#endif

#ifdef CONFIG_CLZ_64
CONST int __clzdi2(uint64_t x) { return clz64(x); }
#endif

#ifdef CONFIG_CTZ_32
CONST int __ctzsi2(uint32_t x) { return ctz32(x); }
#endif

#ifdef CONFIG_CTZ_64
CONST int __ctzdi2(uint64_t x) { return ctz64(x); }
#endif
#line 1 "/home/desw/OSH_Project/sel4_learning/sel4test/projects/sel4test/domain_schedule.c"
/*
...
/* This is a domain schedule that is suitable for the domains tests in sel4test.
 * All sel4test actually needs is for every domain to be executable for some
 * period of time in order for the tests to make progress.
 *
...
/* remember that this is compiled as part of the kernel, and so is referencing
 * kernel headers */
